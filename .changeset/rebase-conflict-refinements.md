...
- **Push-race is closed.** If the target advances between the passing impl gate and the push,
  ...
  gate saw green, even under concurrent merges. The re-verify stays in the conductor/deliver seam. **The loop is bounded, not forced** — a target that keeps advancing past a small cap of
  attempts is a liveness stop: the conductor stops and escalates (records a `halt`) rather than
  spinning forever.
