...
Run the golden set via aces-judge over N runs and report pass rate + per-layer breakdown.
...
the project-wide health roll-up (`report`); how a single case is scored (that is `aces-judge`).
...
| Score the golden set | a resolved suite (`eval.md` + `golden-set/`) and the target config | every case is judged by `aces-judge` and collapsed to a pass/fail; all cases run before any report |
