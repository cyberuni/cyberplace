...
Feature: implementer — the impl-judge role
  ...
  Scenario: dispatched as the impl-judge it runs the evals
    Given the conductor dispatches implementer with a subject and its eval suite keyed to frozen scenarios
  ...
  Scenario: it does not author the evals
    Given the eval suite handed to implementer was written by the impl-producer
  ...
  Scenario: it never edits the spec or the feature
    Given implementer is running the eval suite
  ...
  Scenario: it delegates per-case scoring to judge
    ...
    When implementer evaluates that scenario
  ...
  Scenario: one eval per frozen scenario is confirmed
    ...
    When implementer loads the eval suite
  ...
  Scenario: each scenario collapses to one boolean
    ...
    When implementer collapses the result
  ...
  Scenario: all scenarios passing rolls up to a passing gate
    ...
    When implementer rolls up the gate verdict
  ...
  Scenario: a behavior-changing gap is a blocker not an edit
    Given implementer finds a behavior-changing gap while running the suite
