...
then hand it to the ACES eval loop to spec and score.
...
and point the user at the ACES eval loop to spec and score it.
...
scoring a config or adding eval cases (`run` / `add`); the skill-quality rubric itself (the
...
| Hand off to the ACES eval loop | a completed skill that carries triggering or graded behavior | the report names the SKILL.md path, README, runtime symlinks, and audit outcome, and points the user at `start-mission` (or `add` / `run`) to spec and eval the skill rather than embedding a legacy trigger-query test |
| Co-produce the eval suite as impl-producer | dispatched by the conductor as the ACES impl-producer against a frozen `.feature` | the SKILL.md and an eval suite carrying one eval per frozen scenario are produced together |
