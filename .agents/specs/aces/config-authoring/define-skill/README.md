...
| Hand off to the ACES eval loop | a completed skill that carries triggering or graded behavior | the report names the SKILL.md path, README, runtime symlinks, and audit outcome, and points the user at `start-mission` (or `add-scenario` / `run`) to spec and eval the skill rather than embedding a legacy trigger-query test |
