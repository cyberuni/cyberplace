...
Feature: define-skill — author a workflow skill
  ...
  hand it to the ACES eval loop to spec and score. Authoring an agent definition or persona is
  ...
  # ---- Handoff to the ACES eval loop ----

  Scenario: the report names the artifacts and points at the ACES eval loop
  ...
  Scenario: no legacy trigger-query eval file is embedded as the test step
    ...
    When define-skill reports
    Then it does not embed a legacy trigger-query eval file as the test step and defers scoring to the ACES eval loop

  # ---- Impl-producer dual mode ----

  Scenario: dispatched against a frozen suite it co-produces the eval suite
    Given the conductor dispatches define-skill as the ACES impl-producer against a frozen feature
