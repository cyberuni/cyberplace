...
| Keep a boolean-decided property out of the rubric | a subject whose suite already carries a boolean scenario deciding a property | it does **not** add a dimension re-grading that property; it leaves the property to its boolean scenario rather than smuggling an untradeable boolean into the compensatory sum (`../../design/test-levels.md`) |
