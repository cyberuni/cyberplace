...
| Name a gate/case-scorer agent by its role | the agent being authored has the role of scoring or verifying a specific gate or case (vs. a producer/worker named by its action) | the agent is named for the gate + scope it serves — `<domain>-<gate>-judge` (e.g. `sdd-impl-judge`) or `<domain>-case-judge` (e.g. `aces-case-judge`) — and a bare action-verb name (`implementer` / `judge` / `validator`) is flagged HIGH by the gate-role naming check and corrected, while a non-scorer producer keeps its action-oriented name |
...
| Co-produce the eval suite as impl-producer | dispatched by the conductor as the ACED impl-producer against a frozen `.feature` | the agent definition and an eval suite carrying one eval per frozen scenario are produced together |
