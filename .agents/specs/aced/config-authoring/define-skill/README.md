...
| Name a gate/case-scorer subagent by its role | the subagent being authored (a partial skill) has the role of scoring or verifying a specific gate or case (vs. a producer/worker named by its action) | the subagent is named for the gate + scope it serves — `<domain>-<gate>-judge` (e.g. `aced-impl-judge`) or `<domain>-case-judge` (e.g. `aces-case-judge`) — and a bare action-verb name (`implementer` / `judge` / `validator`) is flagged HIGH by the in-skill gate-role naming check and corrected, while a non-scorer producer keeps its action-oriented name |
