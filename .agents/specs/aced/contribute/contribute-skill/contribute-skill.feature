@frozen
Feature: contribute-skill — contribute a locally-improved installed skill back to its source repo
  Unit suite for the contribute-skill skill: route a "PR this improved skill back upstream" request, resolve
  ...
  @trigger
  Scenario Outline: contribute-skill activates on a contribution request and defers its siblings
  ...
  Scenario: a request to PR an improved skill in this repo's own tree does not fire (repo-native carve-out)
    ...
    When ACED routes the request
    Then contribute-skill does not handle it because contributing to a skill's own source repo is ordinary in-repo work
  ...
  Scenario: a request to scaffold a new skill from scratch defers to define-skill
    ...
    When ACED routes the request
    Then contribute-skill does not handle it and define-skill does
  ...
  Scenario: a request to generalize the current session into a skill defers to skillify
    ...
    When ACED routes the request
    Then contribute-skill does not handle it and skillify does
  ...
  Scenario: a request to diagnose an installed skill's failing evals defers to improve
    ...
    When ACED routes the request
    Then contribute-skill does not handle it and improve does
  ...
  Scenario: a consumer .agents/skills path maps to the source skills/<name>/ tree
    ...
    When contribute-skill maps the file to its upstream path
  ...
  Scenario: every changed file under the skill folder is contributed, not only SKILL.md
    ...
    When contribute-skill collects the files to contribute
  ...
  Scenario: the diffs are shown and confirmed before any push
    ...
    When contribute-skill prepares to contribute
  ...
  Scenario: an unchanged skill contributes nothing and stops
    ...
    When contribute-skill compares the local skill to the source
  ...
  Scenario: the PR URL and the consumer refresh step are reported
    Given contribute-skill has opened the pull request
  ...
  Scenario: no path outside skills/<name>/ is written in the source
    ...
    When contribute-skill builds the contribution tree for the source repo
  ...
  Scenario: nothing is pushed before the diffs are confirmed
    ...
    When contribute-skill reaches the push step
  ...
  Scenario: the produced PR is scoped to the source skills tree on the correct base branch
    ...
    When contribute-skill opens the pull request
