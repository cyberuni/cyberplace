...
Feature: operator — the command-center persona
  ...
  Scenario: the headless realization runs Operator's dispatch flow with no live Council
    ...
    Then the headless-operator agent runs the same out-of-ship dispatch Operator runs in-session,
      carries no logic Operator plus the mission-graph engine do not already hold, and batches anything
      it cannot decide up its relay rather than asking live
  ...
  Scenario: the loop pulls the ready frontier and dispatches the top-ranked mission
    ...
    Then it reads the frontier from the mission-graph engine's ready query and dispatches the
      highest-ranked mission it has capacity to run
  ...
  Scenario: a mission is claimed on the graph before it is spawned
    ...
    Then it first appends a claim to the mission graph (status in-progress) as the single writer, then
      runs cyberlegion unit spawn for the ship that will execute it
  ...
  Scenario: capacity and human-availability gate what actually runs
    ...
    Then it runs at most K at once, sends an AFK mission to an autonomous ship and a HITL mission to a
      human channel, and leaves the rest on the frontier for a later tick
  ...
  Scenario: the Operator is the sole graph writer; dispatched missions only report
    ...
    Then the dispatched mission never writes the graph itself, and the headless-operator appends the
      retirement so claims and retirements never race
  ...
  Scenario: completion retires in Operation order and re-derives the next frontier
    ...
    Then it merges in Operation order behind the merge backstop, tears down the pod that ran it, appends
      the retirement and any discovered edges or nodes as the single writer, and re-derives ready to
  ...
  Scenario: the loop's spawns are inter-mission, distinct from Pod's intra-mission fan-out
    ...
    Then those spawns stay Operator's inter-mission dispatch, separate from Pod's intra-mission
      worktree fan-out inside a single ship, and no rule of the in-ship Pod persona is invoked
