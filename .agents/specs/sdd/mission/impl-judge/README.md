...
| **two test levels (deterministic domains)** | confirm every frozen acceptance scenario passes at the boundary **and** the inner-rule combinatorial space has unit coverage; missing inner-rule coverage is a finding **distinct** from a per-scenario acceptance failure (`../../authoring/suite-format/README.md`) |
| **never demand the feature enumerate combinatorics** | never fail an intent for leaving its inner-rule combinatorics to unit tests; the `.feature` is not required to carry one scenario per combinatorial case — the churn this doctrine ends |
| **no inner layer, no inner-rule bar** | a domain with no deterministic inner layer (a graded non-deterministic subject) is **not** held to inner-rule unit coverage; its graded behavior is judged through the suite's `@rubric` scenarios instead |
