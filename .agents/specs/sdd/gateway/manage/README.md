...
dispatcher**: it classifies a manage request and **loads the matching corpus engine in the current
...
audit / housekeeping) and **load the matching corpus engine in the current session**, so the session
...
| **audit → corpus engine** | an audit request loads `check-spec-structure` / `align-spec` / `formation` |
...
| **Inspect** | `discover-specs` (list specs + statuses) · `concept-index` (by-concept view) · `place-node` (where a concept belongs) · `discover-plans` (in-progress missions) — the read-only corpus engines (`../../corpus/`, `../../intake/plan-discovery/`) |
