...
| **setup → backfill** | a "set up the project spec for the first time" request loads `scaffold-project-spec` |
...
flowchart TD
    ...
    PICK -->|Setup &amp; discovery| SD["scaffold-project-spec · manage-spec-anchors · manage-scenario-bridge · init"]
...
|---|---|
| **Setup & discovery** | `scaffold-project-spec` — scaffold a project's spec envelope for the first time (`../../authoring/scaffold-project-spec/`) · `manage-spec-anchors` — list / CRUD / induce / preview discovery's extra spec anchors (`../../corpus/spec-anchors/`) · `manage-scenario-bridge` — list / scaffold / add a project's `scenario-bridge.toml` sources (`../../mission/manage-scenario-bridge/`) · `init` — the onboarding front door; wire SDD's opt-in conveniences, v1 the mission statusline (`../init/`); these are prerequisites for a project being found and usable, not routine cleanup |
...
engine** — `scaffold-project-spec` scaffolds the skeleton, `plan-retirement` performs its gated
