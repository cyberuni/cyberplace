...
Most are deterministic and read-only; **`align-spec`** is the exception — **user-invocable** (and
CI-usable) and able to **reconcile**, not just report. This folder is the *machinery*; the
[`../formation/`](../formation/README.md) outer loop is the **process** that wields it (audit
node-shape, split an oversized node, reconcile drift or contradiction). The tools compute and
propose; formation — run by the Warden — decides and acts, making its own self-clear-vs-escalate
verdict per act.
...
units). Because one project is one spec, the old **cross-spec** structural acts — deduping sibling
specs, splitting one spec out of another — are **retired** (see *History* in
[`../corpus/README.md`](../corpus/README.md)). What remains is **intra-spec** maintenance:
`concept-index` re-unifies a scattered concept, `place-node` advises a new node's home,
`check-spec-structure` audits node-shape, `digest` summarizes, and `align-spec` reconciles
prose↔suite drift. None reads runtime behavior or owns a lifecycle state; none **gates** — they
advise the Warden or reconcile prose/scenarios.
...
`../design/lifecycle-model.md`, not a maintenance tool.)
...
| Unit | Type | Spec | Role |
|---|---|---|---|
| **digest** | behavioral | [`digest/`](./digest/README.md) | a read-only, fixed-section summary of one project-spec (What / Status / Scenarios / Key decisions / Open items) |
...
- **Tooling, not process.** These tools compute and propose. The act — splitting an oversized node,
  reconciling a contradiction — belongs to [`../formation/`](../formation/README.md), which runs
  continuously and owns the ratification verdict. The per-spec structural verdict at a gate is a
  separate thing.
- **No lifecycle ownership.** Nothing here writes `status`, `approval`, or a freeze. digest,
  concept-index, place-node, and check-spec-structure are read-only and emit views or proposals.
  `align-spec` may write **prose / scenarios** in reconcile mode, but still never
...
- **Authoring and retirement stay elsewhere.** Scaffolding a node is `../authoring/` (explore);
  `deprecate`/retire is a lifecycle act (`../design/lifecycle-model.md`). No maintenance tool writes
