...
Feature: The place-node procedure — suggest a provisional home and catch duplicates
  ...
  Scenario: a concept's home is where its facets already sit
    Given a project-spec where concept "resolution" has nodes under design/ and mission/
  ...
  Scenario: candidate homes are ranked by how many facets sit there
    Given a project-spec where concept "governance" has three nodes under common-governances/ and one under design/
  ...
  Scenario: a concept with no prior home suggests nothing
    Given a project-spec where no node carries concept "telemetry"
  ...
  Scenario: an overlapping name surfaces the existing node
    Given a project-spec containing a node named "resolution"
  ...
  Scenario: a unique name surfaces no duplicate
    Given a project-spec with no node whose name overlaps "leash"
  ...
  Scenario: the suggestion consults no stored routing list
    Given a project-spec whose concept tags are the only placement source
  ...
  Scenario: place-node writes nothing
    Given a project-spec and a node to place
