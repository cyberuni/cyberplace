...
| **run structural provenance/alignment checks** — before any verdict | the touched files' `produced-by` entries + role resolution (`../../design/provenance-model.md`) | malformed `produced-by` / off-enum `correction` / unresolvable required role → **fail closed**; uninstalled-but-valid recorded producer → **flag** only |
