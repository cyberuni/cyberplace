...
1. **Next action — finish `sub-authoring`** (in `authoring/`, applying the worked **overview +
   ...
   Upstream, land first: `model-refinements` (a)-(d) — intake-creates-plan, explore-builds-impl,
   unit-of-work-vocabulary sweep across `loops.md` / `intake/` / `mission/`. Then the remaining
   sub-missions in todo order (`sub-intake`, `sub-gateway`, `sub-mission`, …), same split each.
...
4. **Do not relearn** — the working method and settled calls live in `## Resolved decisions`:
