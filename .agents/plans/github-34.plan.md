...
todos:
  ...
  - id: sub-handoff
    ...
    status: completed
  - id: sub-corpus
    content: "Sub-mission corpus/ — SUITES DONE (2026-06-29, commit e102876). Restructured corpus/ to an index + 5 behavioral units, each README (## Use Cases) + boolean .feature: discovery, digest, dedupe-specs, split-spec, align-specs. Propose-only tools (dedupe/split) own analysis only — the act lives in formation/; align-specs is the user-invocable detect+reconcile tool. NO spec-graph/DAG. Cold sdd-spec-judge ALIGNED on all 5 (closed 3 coverage gaps). IMPLS in sdd-new still pending (deliver). Frozen at the spec gate (7ab8c98)."
    status: completed
  - id: sub-campaign
    ...
    status: pending
  - id: sub-formation
    content: "Sub-mission formation/ — SUITE DONE (2026-06-29, commit 49c82b0). Promoted formation/ to a behavioral node + formation.feature: the three corpus-wide acts (split/dedupe/reconcile, each happy + negative mirror), corpus-wide-vs-gate distinction, the Warden's five self-clear-vs-escalate verdict branches, the frozen-contract guard, input/write boundary, altitude routing. Cold sdd-spec-judge: Builder failed first pass (station-named Then + missing negative mirrors); fixed → ALIGNED. The Warden AGENT (impl) still pending (deliver). Frozen at the spec gate (7ab8c98)."
    status: completed
  - id: sub-doctrine
  ...
  - id: sub-forge
    ...
    status: pending
  - id: sub-plugin
    content: "Sub-mission plugin/ — SUITE DONE (2026-06-29, commit 7170db1). Promoted plugin/ to a behavioral node + plugin.feature covering the three in-scope entry points: ships-as-plugin manifest, workspace init (the .agents/plans dir + the .cursor/plans → ../.agents/plans relative symlink, idempotent + no-clobber), and the contract-registry init-WRITE (idempotent, fail-closed on malformed JSON, version reconciliation, legacy→squads rewrite, preserve other entries). plugin/plugin-contract stays the reference node; the deferred management/marketplace surface stays unscenarioed (Tier-4). Cold sdd-spec-judge ALIGNED. IMPL still pending (deliver); workspace-init impl tracked under sdd-init-skill. Frozen at the spec gate (7ab8c98)."
    status: completed
  - id: sdd-init-skill
    content: "NEW (raised 2026-06-29, dogfood-surfaced) — an SDD `init` skill (workspace init; sibling of registry init-WRITE under sub-plugin). Owns one-time per-repo workspace setup the provenance contract ASSUMES but nothing writes today: the `.gitattributes` `merge=union` rule for `*.log.jsonl` + `ledger.jsonl` (combat-log-governance line 41 assumes it; absent today -> parallel appends conflict on merge), the `.agents/plans` symlink gotcha, and the `.agents/sdd/` resolution-state dir. Surfaced when the sub-doctrine dogfood created the first real `github-34.log.jsonl` and found no merge=union rule. Decide: its own user-facing `init` entry vs folded into sub-plugin workspace-init. Net-new (no baseline) -> likely a follow-up CR, but the .gitattributes gap is real now."
    status: pending
  ...
  - id: combat-log-location
    content: "DESIGN follow-up (raised 2026-06-28, surfaced by the gateway strategy-count bug). SETTLED: the durable `ledger.jsonl` (root spec sibling; holds `gate` + `strategy`) is distilled by the doctrine loop — keep it. RESOLVED (2026-06-28, user-confirmed): the chatty per-mission `report`/`correction` combat log STAYS a sibling `.agents/plans/<cr-ref>.log.jsonl` file (NOT folded into the plan brief). Rationale: append-only JSONL + `merge=union` + format symmetry with the durable `ledger.jsonl`; still keyed by cr-ref so it retires together with the plan at retro. No edits needed — the spec tree (provenance-model, plugin, doctrine, mission/conductor, gateway), combat-log-governance, and start-mission ALREADY consistently use the sibling form; verified consistent. SEPARATE bug — FIXED (5af5be7): the gateway counts pending strategy from the one root `ledger.jsonl`, not `*.log.jsonl`. COMPLETE."
    status: completed
  - id: combat-log-contract
    content: "DESIGN follow-up (raised 2026-06-28, this session) — sub-doctrine-adjacent; combat-log-location settled only the FILE (the `.log.jsonl` sibling), not its CONTRACT. Premise: once a PR merges the mission session is GONE (possibly multi-machine), so the doctrine loop can rely on the COMMITTED COMBAT LOG ALONE — raw transcripts are rarely available cross-machine/post-merge (the token-waste dimension is the only transcript-backed one). UNSPECIFIED + to design: the entry shapes (report/correction/gate/strategy) exist in combat-log-governance, but the WRITER, WRITE-TIME, and IDENTITY do not. DECIDED (direction): identity = the GIT HANDLE (`user.name`/`user.email`, or a configured SDD handle), NEVER session/machine ids — the log is committed = SAFE-TO-PUBLISH by construction (no hostnames / abs-paths / OS usernames / session tokens); per-entry handle + append-only + `merge=union` makes multi-machine append clean and tells who drafted/ratified what. Ties to the relayed-ratification seam (human-attributed gate writes can't be relayed into a spawned worker). ADJACENT — RESOLVED this session: process ROUTING — a CR targeting *content* → project spec `.agents/specs/<project>`; a CR targeting *process* → the repo-level process spec `<repo-root>/.agents/spec` (singular), simple repo or monorepo alike; NO per-project `## Process` section (ADR-0011 retraction, commit 1cd301b). The repo-process spec `.agents/spec` does NOT exist yet — it is the real artifact owed (build under sub-doctrine / a repo-spec CR); the durable SDD build-method now stranded in retiring plan briefs belongs THERE, not in `.agents/specs/sdd/design/` (that folder is SDD-the-product). ADJACENT — OPEN: CR↔plan cardinality for cross-project work — split the CR if it has a splittable SOURCE (GH issue → sub-issues, each own cr-ref/plan/log), else split the PLAN (1 CR → N plans; a `local-<slug>` direct-prompt CR can only split the plan). Cross-refs: combat-log-location (done), sub-doctrine (pending), combat-log-governance, memory project_sdd_process_vs_workflow.md + project_sdd_relayed_ratification_seam. CONTRACT LANDED (2026-06-28, branch next; approved plan ~/.claude/plans/melodic-swimming-mountain.md) — driver: doctrine is the ONLY combat-log consumer + runs post-merge/cross-machine (transcripts gone), so capture-at-write-time-or-lose-it; counter-force: committed = published-to-history-forever + forge-upstream, so every field is a publish decision. DECISIONS (settled w/ user): (1) SAFE-TO-PUBLISH-BY-CONSTRUCTION FLOOR binding ALL fields (categorical only; never email/OS-username/hostname/abs-path/session-machine-id/secret/code/literal/RAW-NUMBER); permanence is real (deleted-at-retro = tree-only). (2) IDENTITY = per-entry pseudonymous `handle` (report/correction/strategy; gate keeps `by`); resolution = `SDD_HANDLE` env ONLY, else OMIT → fall back to native git commit author; NO git-config read (dropped the git-config variants per user); never user.email; in-file handle ADVISORY, git commit signature is the attestation. (3) WRITE-TIME = per-entry UTC `ts` (ISO-8601), stamped live (only place knowable). (4) MID-FLIGHT `halt` = NEW first-class entry kind (5 kinds now: report/correction/halt → combat log; gate/strategy → ledger); carries phase + categorical why (floor/blast/novelty/confidence); gate `pause` stays the at-gate twin. (5) WRITER FLUSH rule = flush report/correction/halt to the COMMITTED log DURING the mission (unflushed=lost). (6) EFFICIENCY split = coarse categorical efficiency `correction` rides the committed log (a class, NO raw counts, post-merge); numeric token-waste depth stays transcript-only + pre-merge/same-machine; mechanism established, concrete correction-kind/cause NOT seeded (Council-ratified growth). ALSO fixed doctrine README spec→mission terminology bug (user-flagged: 'every spec reach terminal state'→mission). COMMITS (next): 8df3353 (doctrine terminology), 1669e7d (floor+handle+ts), 1724ba1 (halt+flush), f48f0f5 (efficiency split), bf4e7d0 (conductor suite coverage), 9c34399 (README/desc sweep). Swept: provenance-model, combat-log-governance SKILL+README+ref-node, ownership-governance, forge (redaction scope +handle/ts/numbers), conductor README+feature (new combat-log-telemetry concern + halt scenario). pnpm verify green (13/13). REMAINING OPEN (unchanged): the repo-process-spec `.agents/spec` artifact (owed, sub-doctrine), CR↔plan cardinality; READER-SIDE suite coverage (doctrine distills-from-log-alone, forge redacts) waits on doctrine/forge sub-mission suites (don't exist yet)."
    status: pending
  - id: combat-log-timing-concurrency
    content: "DESIGN OPEN (raised 2026-06-29, dogfood-surfaced; sub-doctrine-adjacent; extends combat-log-contract). The sub-doctrine dogfood ran an explore segment and wrote NO combat log — exposing two unsettled questions. (1) WRITE-TIMING + ENFORCEMENT: the contract says flush report/correction live DURING the mission (unflushed=lost), but nothing ENFORCES it — the conductor self-enforces and a session can simply forget (it did). Should the cold judges check it? Tentative NO: the spec-judge / impl-judge are contract-independent (read spec.md+.feature / the impl only) and CANNOT see their own `report` — the conductor writes the report ABOUT a dispatch AFTER the judge returns, so a judge checking 'my report exists' is a timing impossibility. Enforcement belongs to a DETERMINISTIC post-hoc check, not a cold judge: candidates = (a) a gate-time `.mts` in validate-spec that asserts the segment left well-formed provenance (>=1 line, schema-valid, seq monotonic) before allowing advance; (b) a Stop / SessionEnd hook that flags an unflushed segment; (c) the gate verdict line itself (the one thing written AT the gate) as the minimum durable proof. Open: a/b/c or combine; what 'complete enough' means (can't count dispatches statically). (2) CONCURRENCY — three cases: (a) main session vs its subagents WITHIN one mission = NON-issue: producers/judges write nothing, the conductor is the SOLE writer, so one segment = one writer, no race. (b) two sessions, same cr-ref, different areas (same or diff machine) = REAL race: both append to one `<cr-ref>.log.jsonl` and both claim 'next seq' -> seq collision + live-file interleave; `merge=union` only resolves the GIT-merge of divergent branches, NOT two live processes on one file. (c) different machines = seq collides globally (each starts seq 1); `ts`+`handle` become the real order, seq is no longer a per-CR key. DIRECTIONS to weigh (not decided): drop/relax `seq` -> order by `ts` (+ handle tiebreak); OR per-(handle) or per-segment seq namespacing; OR single-writer-per-file discipline = each session writes its OWN shard `<cr-ref>.<handle|segment>.log.jsonl` and doctrine reads the union (merge=union-friendly, no live race, no seq collision) — likely the cleanest. Ties to: combat-log-contract (handle/ts/floor already landed), sdd-init-skill (.gitattributes merge=union), provenance-model, doctrine/scanner (the reader). NARROWED (2026-06-29, user test 'does it impair the doctrine loop?'): doctrine's job = aggregate + count by `cause`; it NEVER keys on `seq` or cross-writer order. So (i) seq collision / multi-machine does NOT impair doctrine — `seq` is intra-writer convenience only; (ii) branch-per-session + `merge=union` is lossless and well-matched to count-by-cause (exact dups collapse=same event; distinct lines survive=real occurrences); (iii) the ONLY genuine impairment is LOSS/CORRUPTION of categorical entries (undercount), and the sole cause of that is TWO LIVE WRITERS ON ONE SHARED FILE (same machine/path) — which the single-writer-per-segment property already prevents intra-mission; the cross-session shared-checkout edge is rare. ROBUSTNESS RULE to adopt: define recurrence as DISTINCT CRs exhibiting a `cause` (each entry carries `cr`), not raw line count -> immune to within-CR dup, seq collision, and merge artifacts. CONSEQUENCE: per-writer sharding drops from 'needed' to OPTIONAL (only the shared-live-file edge); `seq` may stay (intra-writer) or be dropped for `ts`-order — doctrine-indifferent. Remaining real work = (1) write-timing enforcement — RESOLVED + BUILT (2026-06-29, commit bc28563): gate-line-as-floor IMPLEMENTED in check-spec-state.mts (parseLedgerGates + checkGateFloor over each spec dir's sibling ledger.jsonl; status approved/implemented ⇒ matching `gate` approve line required; draft needs no ledger; 7 node:tests; gate-validation-governance prose mirrors it; CLI smoke-tested fail-closed). The live report/correction flush stays WRITE-TIME discipline (append right after each dispatch, where the outcome sits in the Task result), NOT a Stop/SessionEnd hook — SessionEnd too late (content unrecoverable), Stop warm-but-fragile, both CHALKED. (2) recurrence=distinct-CRs — DONE in doctrine/scanner (scanner.feature Given + README six-triggers Input cell; provenance-model can mirror in a sweep). (3) the shared-live-file edge guard (only if two same-machine sessions share a checkout) — still open, low priority."
    status: pending
  ...
  - id: root-frontmatter
    ...
    status: completed
  - id: spec-gate
    content: "Spec gate (Draft -> Approved) — DONE (2026-06-29, commit 7ab8c98, human-ratified by unional). Ran the real spec gate over CR #34's full migration footprint via the sdd-new skills: deterministic check-spec-state + check-feature green tree-wide, cold sdd-spec-judge ALIGNED on every node judged this CR (corpus ×5, formation, plugin, acceptance, doctrine scanner + plan-retirement, mission/conductor 57-scen — the prior hard-floor gap CLOSED), 8 prior nodes carry standing ALIGNED verdicts, no open markers in any touched file. On approve: froze all 24 touched .feature (per-file @frozen), wrote the durable per-CR gate line to a new .agents/specs/sdd/ledger.jsonl, set root spec.md status: approved + approval.spec{verdict:approve, by:unional}. Non-blocking follow-ons recorded (don't gate): scanner token-waste pre-merge path, acceptance section-comment style 4/6, conductor Compatibility pre-auth comment. pnpm verify green (13/13)."
    status: completed
  - id: assemble-plugin
...
> **Session update (2026-06-28, evening) — PAUSED with focus `combat-log`.** A DESIGN detour, no
> migration code. (a) **process-vs-workflow positioning LANDED** — `b88b1f3` (ADR-0011 + spec
> `## What SDD is` / `### SDD vs agentic workflows` + research survey
> `docs/research/sdd-vs-agentic-workflow.md`) and `1cd301b` (retraction of a per-project `## Process`
> section). (b) the **combat-log contract thread** opened as a new todo (`combat-log-contract`,
> pending). Full record: memory `project_sdd_process_vs_workflow.md`.

**▶ THIS PAUSE'S FOCUS — the `combat-log-contract` DESIGN thread.** Runs independently of the
migration build; pick it up as a *process* CR via `start-mission`. The combat log is the ONLY
reliable doctrine input post-merge (session gone / multi-machine) → spec its **writer, write-time,
and identity** in `combat-log-governance`: identity = **git handle** (never session/machine ids; the
log is committed = safe-to-publish), append-only + `merge=union`. Adjacent RESOLVED: process routing
→ the repo-level process spec `<repo-root>/.agents/spec` (NOT a per-project `## Process`); that repo
spec does not exist yet and is the real artifact owed. Adjacent OPEN: CR↔plan cardinality
(split-CR-if-sourced, else split-plan). See the `combat-log-contract` todo for the full capture.

**▶ MIGRATION CRITICAL PATH (unchanged, still the live build) — continues below.**
