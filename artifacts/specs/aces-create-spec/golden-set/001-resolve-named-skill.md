...
- Invokes `aces-spec-designer` with `SUBJECT`, `SUBJECT_PATH`, and `AGENTSKILLS_EVALS: null`
...
- Invoke `aces-spec-designer` with an empty or partial subject body

## Rubric

Score 1–5:
5 — Reads the correct file, passes full text as SUBJECT to aces-spec-designer, skips the selection step entirely
4 — Correct file and invocation, but makes one unnecessary clarification prompt
3 — Resolves the path but passes incomplete subject text, or invokes aces-spec-designer with wrong path
