...
- **Variant taxonomy (Q1) → Resolved.** The membership gates *are* the bound; the count is discovered, not fixed. Verdicts: Explorer (Builder) confirmed; Conductor (Architect) forming; **Scout (Oracle)** added as forming — the Oracle's divergent/discovery variant, by the Explorer parallel; **Strategist has no variant** (its three acts share one cognitive profile → sub-roles, not variants). Named the generative pattern (original): variants come off two axes — divergent/convergent and design-time/runtime. Updated the variants table, Variant glossary row, and downstream-artifacts row.
...
## 2026-06-19 — De-overload the Oracle; fix gate outcomes; clean the loop section

- **Oracle is product intent, not general scheduling.** Deferred work re-enters as its *owning* actor's object (feature → Oracle, refactor → Architect); sequencing it is a **decision rule** (product priority vs rework-cost), not a Oracle call. Fixed the control-loop diagram (deferred → backlog, re-prioritized), the scheduling subsection, the three-loops bullet, the glossary "Scheduling decision," and the How-it-composes chain.
- **`block + none` is not universally a Oracle kill.** It is a kill *by whichever face* rejected the attempt: Oracle kills the intent (abandon), Architect/Builder kill the approach/artifact (restart, goal stands). Fixed prose and the gate diagram.
...
- **Cleaned up "Strategist and the loop":** led with the three distinct loops (cadence / object / owner), folded the single-/double-loop mapping in, dropped the confusing DevEx outer-loop digression, simplified the diagram.
...
## 2026-06-19 — Per-actor sections; sharpen Architect/Strategist boundary

- **What changed:** Restructured "The four actors" into a dedicated subsection per actor (Oracle/Builder/Architect/Strategist), each with motive, object, signature output, variant, and an explicit boundary line to its neighbor. Added a **generalization ladder** (within-feature → Builder/design; across-features → Architect/architecture; across-products → Strategist/curation) keyed on *scope of reuse* and *where the result lives*, with the mechanism distinction (behavior-directly / behavior-through-structure / capability-through-knowledge). Made the Architect **active/governance-setting** (draws the lines ahead of time), not passive. Replaced the three weak Strategist bullets (which described architecture-of-the-corpus) with three accumulate-only acts: selection-for-durability, generalization-across-products, pruning-for-truth. Named the recursion explicitly (organizing the corpus is an Architect sub-act the Strategist also performs).
- **Why:** A boundary review found the Strategist was justified with architectural examples, and "generalization across features" needed a verdict (it is Architect, not Strategist — the abstraction lives in and dies with the product).
- **Conclusion changed materially?** No — Strategist stays a distinct actor; the distinction is now carried by *what the output outlives* rather than by shared pattern-recognition skill.
...
- **What changed:** Compared every borrowed term against its source; fixed places where usage drifted. Outer-loop citation restricted to the inner-loop analogy; double-loop claim softened (Strategist spans both modes); `producer ≠ judge` reframed as an echo of four-eyes plus a time-split of judgment; walking-skeleton recast as contrast not sibling; "stub" generalized to "placeholder"; Strategist confirmed as platform (not enabling) with an enabling echo; Gartner reframed as outcome-vs-capability; Conductor/orchestrator kept distinct (actor vs delegate pattern); added an "Actor" glossary note for the UML/actor-model collision.
