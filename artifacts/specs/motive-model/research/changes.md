...
- **Variant taxonomy (Q1) → Resolved.** The membership gates *are* the bound; the count is discovered, not fixed. Verdicts: Explorer (Builder) confirmed; Conductor (Architect) forming; **Scout (Director)** added as forming — the Director's divergent/discovery variant, by the Explorer parallel; **Strategist has no variant** (its three acts share one cognitive profile → sub-roles, not variants). Named the generative pattern (original): variants come off two axes — divergent/convergent and design-time/runtime. Updated the variants table, Variant glossary row, and downstream-artifacts row.
...
## 2026-06-19 — De-overload the Director; fix gate outcomes; clean the loop section

- **Director is product intent, not general scheduling.** Deferred work re-enters as its *owning* actor's object (feature → Director, refactor → Architect); sequencing it is a **decision rule** (product priority vs rework-cost), not a Director call. Fixed the control-loop diagram (deferred → backlog, re-prioritized), the scheduling subsection, the three-loops bullet, the glossary "Scheduling decision," and the How-it-composes chain.
- **`block + none` is not universally a Director kill.** It is a kill *by whichever face* rejected the attempt: Director kills the intent (abandon), Architect/Builder kill the approach/artifact (restart, goal stands). Fixed prose and the gate diagram.
