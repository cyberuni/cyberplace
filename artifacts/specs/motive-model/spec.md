...
| ------------- | ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| **Oracle**    | **Intend** — what's worth doing                                   | The problem worth solving and what success means; the authority to decide *not* to build                                 |
...
| **Strategist**   | **Accumulate** — make knowledge compound                          | The durable, reusable knowledge every other role draws on                                                                |

The motive is what makes each a real actor: each motive generates use cases the others don't. The Oracle's signature output is a *kill decision*; the Builder's is a *working artifact*; the Architect's is a *boundary or convention*; the Strategist's is *reuse*. Distinct motives, distinct use cases, distinct interfaces.
...
flowchart TB
    ...
    C["Strategist — curation<br/>generalize across products<br/>lives in: the corpus · outlives the product"]
...
- **Boundary (vs Builder):** the Oracle owns *whether and why*; the Builder owns *how*. A Builder who redefines the goal has stepped into the Oracle role.
...
- **Boundary (vs Strategist):** the Architect's abstraction lives in *this* product and dies with it. The moment the output is lifted out as knowledge meant to outlive the product, it is Strategist work.

#### Strategist — accumulate (the foundation tier)

The three roles above operate on the *product*. Strategist does not — its object is **knowledge designed to outlive any single product**. That raises the objection that recurs every time: isn't keeping the corpus organized just *architecture at another tier*?

**Partly — and worth saying plainly.** Organizing the corpus (keeping it coherent, DRY, legible) *is* architecture-of-the-corpus, and a Strategist performs it constantly — the way every actor borrows skills across roles. But the role is named for the part architecture does **not** contain. Three acts are Strategist-only, and none is "organize":
...
These are *accumulate* — grow a compounding, reusable asset — not *structure*. That is the motive architecture lacks, and it is why a corpus-maintainer flipping into Architect-of-the-corpus no more makes Strategist a sub-case of Architect than a Builder reviewing code makes review a sub-case of Building.

Its output *feels* like a layer because every other actor's delegate reads from it — precisely the position of a **platform or infrastructure team**: a role whose *product* is a layer, "made available via self-service capabilities… easy for the [other] teams to consume" [team-topologies]. (Its capability-raising has an *enabling-team* flavor; but unlike an enabling team — time-boxed, product-less — Strategist is permanent and owns a product, the corpus, which is the **platform** pattern [team-topologies].) So the model has two tiers:
...
The Strategist's own practices live in the corpus too, so it is self-describing — a fixed point, not an infinite regress. And a prediction falls out of the tiering: infrastructure is the first thing a team neglects, and its neglect degrades *everyone* — a decaying corpus forces every delegate, in every role, to start cold.

### Every actor has two faces: produce and evaluate

The same expertise points two ways. Applied **forward**, it produces — a Builder writes code, an Architect draws structure, a Oracle frames a problem. Applied **backward**, the same expertise *judges* — the Builder reviews code, the Architect runs a fit-to-structure review, the Oracle makes a kill-or-ship call. Nothing about the person or their knowledge changes between the two; only the *orientation* does. This is "verbs, not titles" at its sharpest.

*The same expertise, pointed two ways — one **motive** (the want) pursued forward and backward:*

| Actor         | Motive — the want                       | Forward face — produce            | Backward face — evaluate         |
| ------------- | --------------------------------------- | --------------------------------- | -------------------------------- |
| **Oracle**    | the right thing exists                  | frame the problem · set the bar   | kill-or-ship call                |
| **Builder**   | the thing works                         | generate the part                 | **validate** against the bar     |
| **Architect** | the whole stays coherent and evolvable  | draw the structure                | **fit-to-structure review**      |
| **Strategist**   | knowledge compounds                     | distill into the corpus           | prune for truth                  |

The motive is **face-invariant**: the forward face produces *toward* the want, the backward face judges *against* it. "Generate," "structure," and the rest name the *forward* face — not the motive, which both faces serve.

So there is **no standalone Gatekeeper actor.** "Gatekeeping" is what any actor's expertise does when turned backward; a thing with no domain of its own is an orientation, not a role. The word survives as the name of an *activity*, not a party.

**The evaluate face is productive, not passive.** A backward face does not merely observe — it has two outputs: (a) it **codifies the bar** into durable, loadable criteria (which forward producers can load to self-align before the gate fires), and (b) it **produces the verification** — executes the criteria against the artifact, generating the evidence — then renders the verdict. Codifying the bar is itself authorship; the criteria are a delegation surface the actor builds so judgment runs without the actor being present.
...
**Grader-independence comes from the bar or the orthogonal axis — not the produce/evaluate agent-split alone.** For a *single* actor, its produce face and its evaluate face share the same lens and criteria — they are two orientations of *one* expertise. Splitting them across two agents is only anti-tampering hygiene (it stops the crude cheat of editing a test to go green); it does *not* yield an independent perspective. If an actor misreads the artifact it was judging, its produce-face output and its evaluate-face output embed the same misread — same lens, same blind spot, same outcome. Real independence requires one of two things: **(a) an independent bar** — the criteria the evaluate face runs were set by a *different* actor's product, not self-set, so the evaluator is anchored to a contract it did not author; or **(b) the orthogonal axis** — a *different* actor's backward face judging a property the producer was not optimizing (see *Delegate fidelity: the orthogonal axis*), catching the producer's blind spot from outside its angle. The agent-split is therefore a *necessary* hygiene condition, not a *sufficient* independence condition.
...
**The gate** is the boundary — a pull request, a release — where backward faces converge on one change: correctness (Builder, backward), fit-to-structure (Architect, backward), and worth-shipping (Oracle, backward) are judged together. What comes out is **not a single bit.** It is two decisions on two axes:
...
Two corners earn names. `block + none` is a **kill** — no incremental fix saves this attempt — but *what* dies depends on the rejecting face: a **Oracle** kill abandons the goal (*this should not exist*), while an **Architect** or **Builder** kill rejects the approach or the artifact (*not like this*) with the goal still standing. How often it fires at all depends on how *editable* the upstream is: when the artifact is regenerated from a spec (spec-driven development), an Architect or Builder rejection usually becomes *revise the spec and retry* — an iteration, not a kill — leaving the **Oracle** kill (a goal no spec edit saves) as the main residual. `accept + deferred` is the **feedback edge**: merge now, spin off work that re-enters the loop later (see *How it composes*).
...
The first axis is governed by a **decision rule** — how the backward faces combine into one verdict, anywhere from an all-pass unanimous veto to a single senior decider (in practice, often a senior engineer weighting the Architect face, or a domain expert weighting the Oracle face). The rule is *governance*, a policy choice, not an actor. The second axis is *generated output*, not a combination rule — so the two stay distinct.
...
A second honesty about timing: **the defer verdict crystallizes at the gate, not mid-iteration.** A deferred change request asserts two things at once — that a structural concern is *real*, and that it *survives this change unfixed*. The second claim holds only once the artifact stops moving; while iteration continues, a later pass may delete the very code the concern was about. So an Architect's structural discovery *during* iteration is a **candidate**, not a committed change request — it crystallizes, or evaporates, only when iteration converges at the gate. This is the **editability** mechanism from the verdict axis (above) applied to the change-request axis: editability dissolves premature *kills* and premature *deferrals* alike — same cause, both axes.

#### `accept` is a strength, and gates firm the ends of a co-delivered chain

**`accept` is a commitment strength, not an absolute lock.** Passing the gate raises an artifact's standing; it does not make it irreversible. A later deal-breaker — a backward face that only fires once consequences are visible — can still send an accepted artifact back, the editability mechanism running in reverse. So `accept` sets *how firmly* the artifact is held, and the firmness varies: an artifact close to settled intent is held hard; one close to fresh generation stays provisional. A gate does not freeze; it **firms**.

**A production's artifacts are co-delivered, and each gate firms one end of the chain — gates do not slice the chain into sequential phases.** An actor-chain yields several artifacts together — intent, structure, realization — that move as one. Gating each artifact behind its own checkpoint before the next may begin is the waterfall failure; it breaks co-delivery and forfeits the editability that dissolves premature kills. Instead a production may carry more than one gate, each setting the commitment strength at *its* end: an early gate firms the intent/contract end while the realization end stays provisional; a late gate firms the realization end against the now-firm contract. The middle stays soft and adaptable — committed, but at lower strength. This is the **last responsible moment** expressed as a gradient rather than a wall: strength descends from the settled end of the chain to the freshly generated end, and each gate raises it only where its faces can responsibly judge.

#### The deferred branch is a scheduling decision over a dependency tree

`accept + deferred` is richer than "file a ticket." It emits **new work that re-enters as its owning actor's object** — a deferred *feature* is a Oracle concern (product intent), a deferred *refactor* an Architect concern (structure). **Sequencing it against everything else is a decision rule** — governance weighing the Oracle's product priority against the Architect's structural urgency, not one actor's call. The **Architect** detects a structural concern and estimates it (scope, complexity → rework cost); the rule weighs that estimate against product priority to set the order.
...
| ------------ | ------------------------------- | ---------------------- | ---------------------------------------------------------------------------------------------------------- |
| **PM**       | product · intent                | Oracle · forward       | Builder (prototype with a delegate), Strategist (encode product knowledge)                                    |
| **Designer** | product · UX                    | Builder · forward      | Oracle (own the *why*), Architect (the design system), Strategist (the design corpus)                         |
| **Engineer** | product · code                  | Builder · forward      | Architect (structure), Strategist (conventions); reaches process & toolchain                                  |
| **QA**       | product · quality               | Builder · **backward** | Architect (acceptance criteria), Strategist (golden sets & regression corpus), Explorer (adversarial probing) |

QA is the sharpest case: its default is the **backward-face Builder** — the trained quality specialist. When delegates write the checks, QA stops being the one who *writes* them and becomes the one who *owns the acceptance contract and curates the corpus it draws on* — backward Builder plus Architect and Strategist.
...
- **Persistence** — the need must *recur and aggregate* into a standing demand a human specializes toward. (Not continuous occupancy — a Oracle's kill is a split-second act; the *need* is what must aggregate.) A momentary, absorbable need is a sub-role: a *janitor*, not a *cleaner-of-the-flush-handle*.
...
| ------------- | ---------------------------- | ------------------------------------------------------------------------------------- |
| **Oracle**    | **The brief**                | The problem, the *why*, the definition of success                                     |
...
| **Strategist**   | **The corpus**               | Distilled, reusable knowledge every other delegate reads from                         |
...
- **Inner loop** — *within a task.* Builder produces, the **bar** (a backward face made executable) fires, Builder corrects, Architect reshapes under green. Fast; every iteration — the developer inner loop [dev-loop]. This is **single-loop** learning [argyris]: correcting *actions* under fixed assumptions. The Strategist does not fire here.
- **Product feedback edge** — *across tasks, same product.* Work deferred at the gate re-enters as its owning actor's object (a refactor → Architect, a feature → Oracle), sequenced by a decision rule. New work, same product.
- **Outer loop** — *across products.* The Strategist distills durable lessons into the corpus so the next inner loop starts warmer. This is **double-loop** learning [argyris]: revising the *assumptions* themselves.
...
The middle and outer loops are easy to confuse but distinct: deferred work changes *this* product; the Strategist changes the *capacity to build any product*. And the Strategist spans both learning modes — routine **codification** (encode what works — a lint rule for a thrice-solved pattern) is single-loop; **revision** (prune a now-false convention, resolve a contradiction) is double-loop, its distinctive mode.

Firing Strategist every iteration is **premature codification** — you encode transient noise and thrash the corpus before you know which lessons are durable. So the human Strategist is episodic, triggered at boundaries: a pattern solved three times, the same correction repeated across loops, a contradiction or staleness that needs pruning, a milestone retro, or a **lifecycle terminating** — a product or effort that *ships* (distill what worked) or is *killed* (distill *why it failed* — often the richest lesson, pairing with the Oracle's highest-leverage kill). The trigger is **lifecycle granularity**, not per-gate: a single PR shipping is not an outer-loop trigger — that is the premature codification just named.

The interface this produces is the model's first concrete one: the **Strategist's delegate watches continuously** — flagging candidates, drafting conventions, detecting corpus contradictions, all cheap — while the **human Strategist holds the accept/prune decision**, because that call is accountable and high-blast-radius. *Detection and drafting by the delegate; keep-or-cut by the human.* It is a template for every other actor's interface.
...
> Building a robust fidelity system for one class of delegate — agent configurations — is itself a product, with its own Oracle, Builder, Architect, and Strategist. The framework describes how such a product gets built; it does not contain it.
...
- **The gate** — the maintainer enters *only now*, turning expertise backward against the change: correctness (Builder face) against the bar, fit-to-structure (Architect face), and worth-merging (Oracle face). The two-axis verdict applies: accept-or-block, with or without change requests — and a recurring-class concern may be `accept + deferred`, filed as a follow-up. Note the asymmetry — the contributor produced for days with no evaluation present; the gate is async and late.
- **Strategist** — if the bug is one of a recurring class, the maintainer encodes a lint rule, a test pattern, or a CONTRIBUTING note so the next contributor avoids it. Frequently deferred or skipped — infrastructure neglect in the wild.

Reading: one human (contributor) plays Oracle + Explorer + Builder forward; another (maintainer) judges at the gate — the Builder, Architect, and Oracle faces turned backward — and curates. Roles cluster by *position in the contribution flow*, not by job. And the same maintainer who judges here is a Builder on their own commits — the verb, not the person.
...
| Trigger | Inputs | Outcome | Scenarios |
|---|---|---|---|
| An agent reads the **actor definitions** | the generated governance | it has Oracle, Builder, Architect, Strategist as typed definitions — each with motive, object, signature output, delegation surface, bar criteria, and the boundary to its neighbors | Machine artifact enumerates the four actors as typed definitions |
...
- **The Architect's breadth (codebase + process + harness) is one motive, not one big role.** It is the Architect motive worked across three **overlapping sets** (product, process, toolchain); see *Recursion*. That set split and *tier* (delivery vs foundation actor, within one build) are different things; they meet at the **Strategist** — the foundation tier within a build and the actor that carries knowledge between the sets.

- **Strategist is peer actor and layer — not a false choice.** *Actor* asks whether a distinct motive generates distinct use cases (it does); *layer* asks where the output sits in the dependency graph (foundational). Strategist is the team's **infrastructure actor**: a full actor whose product is a layer, like a platform team [team-topologies]. The model is two-tiered — three delivery actors over one foundation actor — not actors plus a non-actor substrate.

- **No standalone Gatekeeper actor.** Judging is expertise turned backward, so *every* actor has an evaluative face — there is no role with judging as its own domain. **The gate** is the boundary where those backward faces converge; the verdict is a two-axis decision (accept/block × change-request) governed by a **decision rule** (unanimous veto → single senior decider), i.e. governance, not a party. "Gatekeeper" survives as the name of the *activity*. `producer ≠ judge` holds per artifact across time [sod].

- **Variant count is discovered, not fixed.** The **membership gates** are the answer: the count is not fixed, it is *discovered* per candidate. A candidate is a named variant only when it shares its actor's motive yet demands a *substantial, distinct* body of preparation (capacity gate) for a *recurring, aggregated* need (persistence gate); fail either and it is a sub-role or a function to **codify**, not a variant. Verdicts: **Builder → Explorer** confirmed (forward) and **QA** confirmed (backward); **Architect → Conductor** forming; **Oracle → Scout** forming; **Strategist → none** (its three acts share one cognitive profile → sub-roles, not variants).

- **Delegation surface is medium-agnostic.** The surface transmits intent across an *availability gap* — to a delegate now, a teammate next week, or one's own future self. What AI changes is not the surface but two things around it: **economics** (abundance makes delegation the default, so authoring surfaces becomes the main act) and the **fidelity problem** (a new delegate class whose faithfulness cannot be assumed). The surface's generality is load-bearing: one artifact serves human and agent delegates interchangeably.

- **Abundance is a dial, not a switch.** Where generation is cheap (common code, tests, boilerplate) one human spans many roles — the compressed scenario. Where it stays expensive (novel, hard, research-grade generation) the old *title = contribution* equation survives *locally*. Nothing breaks; the same actors and faces apply, only the *compression* changes. The framework is abundance-relative by construction.

- **Delegate fidelity: a delegate-directed check on each surface, not a fifth actor or third face.** It has no motive (fails first membership gate; it is a *relation*, not a party). A face points at the *work*; fidelity points at the *delegate* (the worker) — different objects. The shape: a delegate-directed check riding every delegation surface, the mirror of the bar. The interface *is* the fidelity instrument.

---

## Open questions

1. **How far does abundance actually reach, and how fast?** Whether AI's "abundance" reaches *novel or hard* generation — and how quickly — is empirical and unsettled. Sets how often the compressed scenario is reachable in practice.

2. **Do the *forming* variants harden?** **Conductor** (of Architect) and **Scout** (of Oracle) clear the membership gates in principle but are not yet settled standing roles. Watch whether each crosses from *forming* to *confirmed*, or whether its capacity stays codifiable and folds into a delegate rather than a human variant.
...
| **Actor**                               | The narrow case of a **role**: one of the four motive-distinct base roles — Oracle, Builder, Architect, Strategist — and the human party in the **actor/delegate** pair. Holds accountability, which never delegates. *Collision to note: unlike a UML use-case "actor" (which may be a non-human system) or the actor-model actor, our actor is **always human** — agents are delegates.*                               |
...
| **Motive**                              | The intrinsic **want** that defines an actor — *face-invariant*: both faces serve it. Named by its forward verb for brevity: **intend** (Oracle — the right thing exists), **generate** (Builder — the thing works), **structure** (Architect — the whole stays coherent and evolvable), **accumulate** (Strategist — knowledge compounds). The **backward face** pursues the same want by *judging against* it, not producing toward it.                                                                                                                                                                                                                                           |
| **Direction**                           | What a human supplies to a delegate — and to a teammate or their own future self: a **motive** grounded in an **angle**. Motive alone is half; the angle (expertise) is what tells the human *which* want is worth holding and *how* to judge against it. The unit a human actually holds and transmits, carried across an availability gap by a **delegation surface**. Distinct from **face**, which is the *orientation* of expertise, not what is transmitted.                                                                                                                                                                                                                          |
| **Object**                              | *What an actor's output is.* A Builder's object is a **part**; the Architect's, the **relations between parts**; the Strategist's, **knowledge that outlives the product**. Object — not scope — separates them.                                                                                                                                                                                                         |
| **Face**                                | The *orientation* of an actor's expertise — **forward** (produce) or **backward** (evaluate). Every actor has both, serving one **motive**. *Reserved: never used for anything else — switching motive is switching actor, not face.*                                                                                                                                                                                                       |
| **Variant**                             | A specialization of an actor — same **motive**, differentiated by required *training* — that clears the **membership gates**. Anchors to *either face*: forward (**Explorer** of Builder, confirmed; **Scout** of Oracle, forming; **Conductor** of Architect, forming) or backward (**QA**, the Builder variant that evaluates the product, confirmed). **Strategist** has none (its acts share one cognitive profile). |
...
| **Angle** (domain)                      | The subject-matter expertise an actor works in — security, UX, performance, data, quality. An actor is **angle-agnostic**: it carries no angle of its own; the human supplies one, always acting *from an angle*. Orthogonal to *both* **actor** and **set** (product/process/toolchain) — and not to be confused with either. (Set-names like *process* or *toolchain* are not angles.)                                                                                                                                                                                                                                         |
...
| **Change request**                      | The gate's second axis: `none` / `yes`, with a **timing** (`within-PR` or `deferred`). Generated output, not a combination rule. `block` forces `within-PR`. The `deferred` timing **crystallizes at the gate**, not mid-iteration: a structural concern found while iterating is a *candidate*, since editability may dissolve it before the artifact settles.                                                                                                                                                                                                                                          |
...
| **Delegation surface**                  | The artifact an actor transmits intent through: **brief** (Oracle), **contract + exemplars** (Builder), **shape** (Architect), **corpus** (Strategist). Categories, not products — a team picks the medium.                                                                                                                                                                                                              |
...
| **Tier**                                | The *dependency* split among the four actors *within one build*: **delivery** actors (Oracle/Builder/Architect, act on the object) versus the **foundation** actor (Strategist, acts on the capacity to deliver). Distinct from the product/process/toolchain sets.                                                                                                                                                      |
...
| `[argyris]`          | Argyris & Schön, *Organizational Learning* (1978) — single-/double-loop learning. [infed.org](https://infed.org/dir/welcome/chris-argyris-theories-of-action-double-loop-learning-and-organizational-learning/). *Strategist spans both; double-loop is its distinctive mode, not all curation.*                                                                                                                                                                                                |
