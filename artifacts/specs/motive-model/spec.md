...
A framework for building any product with AI, organized around **motive** rather than job title (PM, Engineer, Designer, QA). Production used to be scarce, so a *title* fixed a person to one kind of contribution; AI makes it cheap and abundant, dissolving that limit — so what a person holds and extends through an AI **delegate** is no longer a fixed contribution but a **direction**: the **role** (a motive) they take on right now, set from their **angle** of expertise.

Because the competence a delegate brings is **codified, not personal**, an actor is no longer capped at their own angle. What you delegate is **direction** — your **motive** (what you want) and your **angle** (which domain) — together with the **judgment** that gives the angle substance: not "security," but "check every user input for script injection," encoded into the delegate's governance and its **bar**. The delegate itself carries no substance; the competence it runs on is a **discipline**, written once into governances and the corpus and wielded by whoever directs it. So a person can make contributions their title never allowed: a Framer can take on the **Builder** role and return a good-enough working artifact — spec, plan, implementation, tests — because the engineer's, designer's, and tester's judgment already sits in the corpus, and a delegate executes it on the Framer's direction. The other actors are not displaced; they enter at the **Gate**, where the contribution is judged against the bar — deliberately late, after a delegate has carried it most of the way.
