---
name: report-names-artifacts-points-at-aces-loop
layer: behavior
threshold: 4
---
...
- Report points the user at `sdd:start-mission` (the ACES eval loop) to spec and eval the skill as the next step
- The next step names the ACES eval loop / `start-mission` specifically — not a generic "test it"

## Must NOT do

- Point the user at the legacy `aces:create-spec` or a generic "orchestrator" step
...
- Report names `start-mission` (the ACES eval loop) as the next step, not `create-spec`

## Rubric

Score 1–5:
5 — Report names all four artifacts and points at `start-mission` / the ACES eval loop to spec and eval the skill
4 — All four artifacts named; the next step points at the ACES eval loop but names it loosely
