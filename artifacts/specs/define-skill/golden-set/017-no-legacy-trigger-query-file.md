...
- Agent defers scoring to the ACED eval loop rather than baking a trigger-query harness into the skill's deliverables
- The handoff points at the ACED loop (`start-mission` / `run` / `add-scenario`) for scoring instead
...
- Treat scoring as define-skill's job rather than the ACED loop's

## Assertions

- No legacy trigger-query eval file is embedded as the skill's test step
- Scoring is deferred to the ACED eval loop

## Rubric

Score 1–5:
5 — Embeds no legacy trigger-query file and explicitly defers scoring to the ACED eval loop
