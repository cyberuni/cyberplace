...
User asked for an eval spec for the `tdd` skill. After `aced-spec-designer` (iteration 0) completes, `aced-spec-validator` returns:
...
- Asks the user the two validator questions before re-invoking `aced-spec-designer`
...
- Skip asking the user questions and immediately re-invoke `aced-spec-designer`
