Feature: SDD Operator — Explore Phase (produce & judge the contract)
  ...
  Scenario: A draft .feature derives explore mode
    Given the operator is about to run a forward producer
    When the .feature is still a draft
    Then it runs the producer in explore mode

  Scenario: A frozen .feature derives deliver mode
    Given the operator is about to run a forward producer
    When the .feature is frozen
    Then it runs the producer in deliver mode

  Scenario: The exploratory loop shapes the spec and probes it by building
    Given the "auth" domain is in the exploratory loop
    When the spec-producer authors the contract and the cold spec-judge grades it
    And the operator also runs forward producers in explore mode
    Then the operator updates the .feature on each spec-producer iteration

  Scenario: The exploratory loop reaches the spec gate on a clean judge pass
    Given the cold spec-judge passes the .feature
    And no open markers remain in spec.md
    When the operator synthesizes the segment
    Then the .feature reaches the spec gate

  Scenario: An unnamed spec-producer is authored inline by the operator
    Given the "auth" domain has no plugin spec-producer and no model-tuned spec-producer named for the slot
    When the operator runs the spec-producer in explore mode
    Then the operator writes the spec.md body and the .feature
    And produced-by.spec-producer records sdd:sdd-operator
  ...
  Scenario: An explore-mode producer builds against the draft, not a frozen contract
    ...
    And the operator carries the impl-producer output forward into the delivery phase
  ...
  Scenario: An explore discovery is judged before it reshapes the contract
    ...
    Then it becomes a proposed .feature change graded by the cold spec-judge
  ...
  Scenario: Explore-mode discoveries feed back as markers
    ...
    And the operator writes an open marker in spec.md and re-runs the spec-producer
  ...
  Scenario: The planner runs in explore alongside the spec, not after a gate
    ...
    And no plan-gate verdict is required before the domain advances past the spec gate
  ...
  Scenario: A spec-producer that writes frontmatter control fields is rejected
    ...
    And the spec-producer writes only the spec.md body and the .feature
  ...
  Scenario: A default-judge domain spawns the cold SDD spec-judge
    ...
    Then the operator spawns sdd:sdd-spec-judge as a cold agent
    And the default judge is not run inline
  ...
  Scenario: The spec gate blocks when the cold spec-judge returns a failing verdict
    Given the cold spec-judge returns a failing verdict for the .feature
    When the operator synthesizes the spec gate
    Then the operator leaves aligned false
    And the spec stays Draft
    And the operator returns the failing scenarios as a blocker
