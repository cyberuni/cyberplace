...
//#region src/bin/cyber-skills.mts
...
if (command === "run-hook") {
	...
	const { registerHooksForSet } = await import(pathToFileURL(path.join(packageRoot, "hooks", "register-agent-hooks.mjs")).href);
	...
	if (verbose) {
		...
		for (const r of results) process.stderr.write(`${r.agent} | ${r.hook} | ${r.status}\n`);
	}
	...
	const { injectCommitDiscipline } = await import(pathToFileURL(path.join(packageRoot, "hooks", "inject-commit-discipline.mjs")).href);
	try {
