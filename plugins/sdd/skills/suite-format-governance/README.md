...
behavior suite is written and judged: boolean Gherkin, the `@rubric` exception, the test-vector rule
for a `Given` (and its swap test), scenario ordering, and the `@frozen` marker.
