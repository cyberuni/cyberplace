...
5. **Self-check the `.feature` form before returning.** Run the deterministic form check — the `check-suite` engine (`scripts/check-suite.mts` in the `spec-gate` skill), the executable form of `sdd:suite-format-governance` — scoped to what you just authored:
   ...
   Exit `1` prints each `✗ <file>: <reason>`. Exit `0` may still print `⚠ <file>: <reason>` **advisory** findings — read them; they are defects the check can prove but whose repair it cannot pick. The one in force today is the **`@trigger` activation contract**: an outline you tag `@trigger` must carry `query` and `should_trigger` `Examples` columns, because the tag routes the scenario into the judge's trigger-run policy and that policy reads exactly those. If your outline is a deterministic, fully-owned decision table rather than an activation case, **tag it `@behavior`, not `@trigger`** — that is the free repair, and it is yours to make now, while the suite is still unfrozen. **Fix every violation** (a non-boolean/hedged `Then`, leaked rubric lingo in an untagged scenario, a missing `Feature`/`Then`, or missing section comments over the sectioning threshold) and re-run until clean **before reporting `STATUS: complete`**. Settling this mechanical bar here spends no cold-judge round on a defect a linter catches every time; the same engine runs fail-closed at the gate (`../spec-gate/`), so an unfixed violation would block there anyway. If `node` is unavailable, self-review against the suite-format bar by hand.
