...
export function composeStatusLine(root: string, mode: StatusLineMode): ComposeResult {
	const settings = readSettings(root)
	const existing = settings.statusLine?.command
	const base = baseToWrap(existing)
	const command = buildCommand(base, mode)
	if (existing === command) return { changed: false, command }
...
// single entry point init's SKILL.md calls on consent.
export function wireStatusline(root: string, mode: StatusLineMode): WireResult {
	const settings = composeStatusLine(root, mode)
...
export function main(argv: string[]): number {
	...
	if (argv.includes('--wire')) {
		...
		const result = wireStatusline(root, mode)
	...
	w('usage: wire-statusline --root <dir> --wire --mode own-line|same-line')
	return 0
}
