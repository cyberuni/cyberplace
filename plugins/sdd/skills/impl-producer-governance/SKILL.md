...
4. **Test levels — acceptance boundary + inner-rule units** (`sdd:suite-format-governance` carries the doctrine). Author the one verification per frozen scenario at the **inner boundary**: substitute the external dependency at its interface seam (the DIP seam) — never the real service, and never a full end-to-end run. The outer boundary is warranted **only when the integration itself is the behavior under test** — the wiring, protocol, or process boundary is what the intent asserts, not an implementation detail reachable behind a mockable seam.
