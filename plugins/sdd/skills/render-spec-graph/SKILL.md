...
description: "Internal skill: regenerate the SDD spec-DAG view (artifacts/specs/graph.md) from blocked-by frontmatter edges. Invoked by create-spec and the orchestrator after blocked-by changes, or to check staleness — not triggered by users directly."
metadata:
...
Render the spec DAG to `graph.md` — a derived Mermaid view + node table built from the `blocked-by` frontmatter edges across every `spec.md`. `blocked-by` is the source of truth; `graph.md` is never hand-edited.
...
|---|---|---|
| `--root <dir>` | `artifacts/specs` | directory whose child folders are specs |
