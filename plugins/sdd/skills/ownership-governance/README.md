...
Loaded via the harness (`Skill`) by every SDD producer and judge (`sdd-scenario-writer`, `sdd-planner`, `sdd-implementer`, and the plugin producers/judges in `aces`/`quill`), by `sdd-orchestrator`, and by the `create-spec` / `validate-spec` skills. It is the most broadly loaded of the lifecycle skills — most producers need only this bundle.
