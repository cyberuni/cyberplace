...
The **Builder** bar at the **impl gate**: does the implementation meet the frozen `.feature`, and is
...
- **The bar is not self-set.** Each check derives from the frozen `.feature` — one per scenario —
...
- **No green-by-tampering.** Passing means the behavior holds, not that a check was edited to pass;
  the frozen `.feature` is never modified to make the implementation conform.
...
1. **The bar is not self-set** — checks derive from the frozen `.feature`, one per scenario; the judge
