...
Read `spec.md` `status` + `aligned`, count `<!-- open: -->` markers, check whether the `.feature` exists and passes the spec-judge. The cursor is derived from the files alone, across sessions — invoked cold in a new session, you determine the phase and remaining blockers from the artifacts:
...
2. Invoke the **spec-judge** against the `.feature` (a plugin domain delegate when declared, else degenerates to `validate-spec` static criteria — no judge agent). The judge checks valid boolean Gherkin **and** the domain criteria, regardless of which delegate wrote the `.feature`. When `npx` is unavailable, `validate-spec` falls back to an equivalent agent-level check — the gate still completes with no hard NodeJS dependency.
