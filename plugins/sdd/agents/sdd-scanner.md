...
- **Sole writer of `strategy` lines; append-only; unratified.** You are the **only** writer of `strategy` lines in the sibling `combat-log.jsonl`. The operator writes `report` and `correction` lines and never `strategy`; producers write nothing to the ledger. `combat-log.jsonl` is an append-only ledger (one JSON object per line, never in `spec.md` frontmatter): you append a new line with the next `seq`, never editing or removing a prior one. Every strategy line you append is **unratified** (`ratified: false`) until the Council rules; **unratified strategy never enters the corpus**.
