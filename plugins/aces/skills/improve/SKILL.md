...
Diagnose failing eval cases and propose targeted edits to the target artifact.
...
| **Conflicting instruction** | Agent followed one rule but violated another in the same artifact |
| **Scope creep** | Agent did more than the artifact specifies |
...
For each pattern, propose a concrete change to the artifact. Show exact before/after diffs — do not describe changes in prose.
...
1. Adding more specific examples to the artifact
2. Lowering the threshold in `eval.md` for that layer if the bar was set too high
3. Splitting the artifact into two narrower configurations
