...
User-invocable. A **thin dispatcher** (the in-session realization of the manage unit, `.agents/specs/sdd/gateway/manage/`): it classifies a manage request and **loads the matching corpus engine in the current session**, holding no production logic, loading no governance, and writing no contract state.
