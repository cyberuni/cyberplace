...
import { findNear, main, parseConcepts, scanCorpus, suggestHomes } from './place-node.mts'
...
test('suggestHomes returns the capabilities where the concept already lives', () => {
	...
	try {
		...
		const homes = suggestHomes(scanCorpus(dir), 'resolution')
...
test('suggestHomes ranks by facet count', () => {
	...
	try {
		...
		const homes = suggestHomes(scanCorpus(dir), 'governance')
...
test('suggestHomes returns nothing for a concept with no prior home', () => {
	...
	try {
		seed(dir, 'mission/resolution/README.md', 'spec-type: behavioral\nconcept: resolution')
		assert.deepEqual(suggestHomes(scanCorpus(dir), 'telemetry'), [])
	} finally {
		rmSync(dir, { recursive: true, force: true })
	}
})
...
test('findNear surfaces an overlapping name and nothing for a unique one', () => {
	...
	try {
		seed(dir, 'mission/resolution/README.md', 'spec-type: behavioral\nconcept: resolution')
		const near = findNear(scanCorpus(dir), 'resolution')
		assert.equal(near.length, 1)
		assert.deepEqual(findNear(scanCorpus(dir), 'leash'), [])
	} finally {
		rmSync(dir, { recursive: true, force: true })
	}
})
