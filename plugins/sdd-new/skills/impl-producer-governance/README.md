...
Loaded via the harness (`Skill`) by the **spawned generic builder** the conductor dispatches for the impl-producer role (recorded `produced-by.impl-producer: sdd:automaton`). The impl-producer is **mechanical and spawned** on every surface — unlike the spec / solution-producer, which the conductor runs inline (the live grill). The grader stays separate: a cold `sdd-implementer` runs the verification this role authored.
