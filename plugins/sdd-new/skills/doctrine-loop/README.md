...
per-gate), reads persisted artifacts post-hoc, and drafts **unratified** strategy to the one
project `ledger.jsonl`. The entry **shape** and the matchable `cause` enum are owned by
