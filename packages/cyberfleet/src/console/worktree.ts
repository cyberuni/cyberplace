...
/**
 * The flagship rule: refuse when a ship's resolved worktree root would be the primary checkout
 * itself. Mirrors firstmate's "refuse unless the path is a real worktree root distinct from the
 * project primary checkout" — a mission is never run in the primary checkout.
 */
export function assertDistinctFromPrimary(worktreeRoot: string, primaryRoot: string): void {
	if (resolve(worktreeRoot) === resolve(primaryRoot)) {
		throw new Error(
			'refusing to run a mission in the primary checkout — spawn a worktree-ship distinct from the flagship',
		)
	}
}
