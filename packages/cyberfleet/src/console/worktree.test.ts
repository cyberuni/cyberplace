...
describe('gitWorktreeAdapter', () => {
	it('add() runs git worktree add against the primary root and returns the new worktree', () => {
		...
		const wt = gitWorktreeAdapter.add(exec, {
			primaryRoot: '/repo',
			path: '/repo/.cyberfleet/worktrees/x',
			branch: 'b',
		})
		expect(calls[0]).toEqual(['-C', '/repo', 'worktree', 'add', '-b', 'b', '/repo/.cyberfleet/worktrees/x'])
		expect(wt).toEqual({ root: '/repo/.cyberfleet/worktrees/x', branch: 'b' })
	})
...
describe('assertDistinctFromPrimary — the flagship rule', () => {
	it('passes when the worktree root differs from the primary', () => {
		expect(() => assertDistinctFromPrimary('/repo/.cyberfleet/worktrees/x', '/repo')).not.toThrow()
	})
