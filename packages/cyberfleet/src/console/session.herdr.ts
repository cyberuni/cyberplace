 ...
 * documented CLI reference (split/run/read/close) but is unverified against a live binary.
 */
export const herdrSessionAdapter: SessionAdapter = {
	name: 'herdr',

	open(exec, opts) {
		const id = exec('herdr', ['pane', 'split', '--current', '--direction', 'right', '--cwd', opts.cwd])
