...
Feature: mail core — durable inter-agent messaging
  ...
  # ── The Bunker: the human's report-up owner inbox (mail bunker) ──
  # A memorable path to the standing owner inbox — the human's durable report-up box, named for the
  # Operator/dispatcher persona. Sugar over --owner; the --owner mechanism itself is unchanged and
  # stays handle-generic, so pre-rename `legate` bindings keep working (back-compat, not a break).

  Scenario: mail bunker lists the Bunker owner inbox
    Given a standing owner registered under handle bunker holding two messages
    When a session runs mail bunker
    Then it lists the bunker owner inbox's two messages, the same as mail inbox --owner bunker

  Scenario: mail bunker falls back to the legacy legate owner inbox when no bunker exists
    Given no standing owner bunker and a standing owner still registered under the legacy handle legate
    When a session runs mail bunker
    Then it lists the legate owner inbox

  Scenario: mail bunker read peeks a Bunker message without acking it
    Given the Bunker inbox holds one unread message
    When a session runs mail bunker read <id>
    Then the message body is printed
    And the message remains unread in the Bunker inbox

  Scenario: mail bunker ack consumes a Bunker message
    Given the Bunker inbox holds one unread message
    When a session runs mail bunker ack <id>
    Then the message moves into the Bunker's read set

  Scenario: mail bunker with no Bunker and no legacy owner errors rather than reading a session inbox
    Given no standing owner bunker and no standing owner legate
    When a session runs mail bunker
    Then it errors that no Bunker owner inbox exists
    And it names how to mint one

  Scenario: the --owner mechanism stays handle-generic after the Bunker rename
    Given a standing owner registered under an arbitrary handle
    When a session runs mail inbox --owner that-handle
    Then it lists that owner inbox regardless of the handle
