...
beforeEach(() => {
	root = fs.mkdtempSync(path.join(os.tmpdir(), 'cyberplace-add-'))
})
...
test('addSkill installs a single skill from GitHub repo', async () => {
	...
	const result = await addSkill('cyberuni/cyberplace:commit', { root })
...
test('addSkill records entry in lock file', async () => {
	...
	await addSkill('cyberuni/cyberplace:commit', { root })

	const entry = getLockEntry(root, 'project', 'commit')
	expect(entry).not.toBeNull()
	expect(entry!.source).toBe('cyberuni/cyberplace')
...
test('addSkill installs all skills when no skill name given', async () => {
	...
	const result = await addSkill('cyberuni/cyberplace', { root })
...
test('addSkill creates symlink in skills/ for project scope', async () => {
	...
	await addSkill('cyberuni/cyberplace:commit', { root, scope: 'project' })
...
test('addSkill does not create symlink for global scope', async () => {
	const home = fs.mkdtempSync(path.join(os.tmpdir(), 'cyberplace-add-home-'))
	try {
		mockFetchInstall([{ name: 'commit' }])

		await addSkill('cyberuni/cyberplace:commit', { root, scope: 'global', home })
...
test('addSkill skips symlink and returns notification when skills/<name> is a real directory', async () => {
	...
	const result = await addSkill('cyberuni/cyberplace:commit', { root, scope: 'project' })
...
test('addSkill installs only skills matching the skills filter', async () => {
	...
	const result = await addSkill('cyberuni/cyberplace', { root, skills: ['commit', 'audit-skill'] })
	expect(result.installed).toHaveLength(2)
...
test('addSkill installs from bare HTTPS git URL', async () => {
	...
	const result = await addSkill('https://github.com/cyberuni/cyberplace', { root })
	expect(result.installed).toHaveLength(1)
	expect(result.installed[0]!.name).toBe('commit')
})
...
test('addSkill with bare git URL stores normalized cloneUrl as lock source', async () => {
	...
	await addSkill('https://github.com/cyberuni/cyberplace', { root })

	const entry = getLockEntry(root, 'project', 'commit')
	expect(entry!.source).toBe('https://github.com/cyberuni/cyberplace')
...
test('addSkill with GitHub browser URL stores source with #branch', async () => {
	...
	await addSkill('https://github.com/cyberuni/cyberplace/tree/develop', { root })

	const entry = getLockEntry(root, 'project', 'commit')
	expect(entry!.source).toBe('https://github.com/cyberuni/cyberplace#develop')
	expect(entry!.sourceType).toBe('github')
})
