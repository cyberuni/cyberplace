...
export interface CyberplaceLock {
	version: 1
	skills: Record<string, LockEntry>
}

export function getLockPath(root: string, scope: Scope, home?: string): string {
	if (scope === 'global') return join(home ?? homedir(), '.agents', 'cyberplace-lock.json')
	return join(root, '.agents', 'cyberplace-lock.json')
}

export function readLock(root: string, scope: Scope, home?: string): CyberplaceLock {
	const filePath = getLockPath(root, scope, home)
	if (!fs.existsSync(filePath)) return { version: 1, skills: {} }
	try {
		const raw = JSON.parse(fs.readFileSync(filePath, 'utf8')) as Omit<CyberplaceLock, 'version' | 'skills'>
...
export function writeLock(root: string, scope: Scope, lock: CyberplaceLock, home?: string): void {
	const filePath = getLockPath(root, scope, home)
