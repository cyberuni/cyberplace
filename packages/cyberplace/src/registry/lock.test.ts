...
beforeEach(() => {
	root = fs.mkdtempSync(path.join(os.tmpdir(), 'cyberplace-lock-'))
})
...
test('getLockPath returns project path', () => {
	expect(getLockPath(root, 'project')).toBe(path.join(root, '.agents', 'cyberplace-lock.json'))
})

test('getLockPath returns global path', () => {
	expect(getLockPath(root, 'global')).toBe(path.join(os.homedir(), '.agents', 'cyberplace-lock.json'))
})
...
test('readLock returns empty lock on malformed JSON', () => {
	...
	fs.writeFileSync(path.join(root, '.agents', 'cyberplace-lock.json'), 'not json')
	expect(readLock(root, 'project')).toEqual({ version: 1, skills: {} })
})

test('writeLock creates file with correct structure', () => {
	writeLock(root, 'project', { version: 1, skills: { commit: sampleEntry } })
	const filePath = path.join(root, '.agents', 'cyberplace-lock.json')
	expect(fs.existsSync(filePath)).toBe(true)
	const parsed = JSON.parse(fs.readFileSync(filePath, 'utf8'))
	expect(parsed.version).toBe(1)
	expect(parsed.skills.commit.source).toBe('cyberuni/cyberplace')
})
