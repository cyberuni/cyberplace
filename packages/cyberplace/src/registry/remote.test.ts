...
beforeEach(() => {
	root = fs.mkdtempSync(path.join(os.tmpdir(), 'cyberplace-github-'))
	cloneSimDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cyberplace-clone-sim-'))
})
...
test('fetchAndInstallSkill copies SKILL.md and sibling files via sparse clone', async () => {
	...
	const spec: RepoSpec = {
		...
		repo: 'cyberplace',
		skill: 'commit',
		raw: 'cyberuni/cyberplace:commit',
	}
...
test('fetchAndInstallSkill installs all skills when no skill name given', async () => {
	...
	const spec: RepoSpec = { type: 'repo', owner: 'cyberuni', repo: 'cyberplace', raw: 'cyberuni/cyberplace' }
	const installed = await fetchAndInstallSkill(null, spec, root, 'main', undefined, cloneSimDir)
