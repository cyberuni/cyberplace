...
export interface CyberplaceConfig {
	version: 1
	providers?: Provider[]
}

export function getConfigPath(root: string, scope: Scope): string {
	if (scope === 'global') return join(homedir(), '.agents', 'cyberplace.json')
	return join(root, '.agents', 'cyberplace.json')
}

export function readConfig(root: string, scope: Scope): CyberplaceConfig {
	const filePath = getConfigPath(root, scope)
	if (!fs.existsSync(filePath)) return { version: 1 }
	try {
		const raw = JSON.parse(fs.readFileSync(filePath, 'utf8')) as Omit<CyberplaceConfig, 'version'>
...
export function writeConfig(root: string, scope: Scope, config: CyberplaceConfig): void {
	const filePath = getConfigPath(root, scope)
	fs.mkdirSync(dirname(filePath), { recursive: true })
	const data: CyberplaceConfig = { version: 1 }
