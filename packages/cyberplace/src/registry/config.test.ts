...
beforeEach(() => {
	root = fs.mkdtempSync(path.join(os.tmpdir(), 'cyberplace-config-'))
})
...
test('getConfigPath returns project path for project scope', () => {
	expect(getConfigPath(root, 'project')).toBe(path.join(root, '.agents', 'cyberplace.json'))
})

test('getConfigPath returns home path for global scope', () => {
	expect(getConfigPath(root, 'global')).toBe(path.join(os.homedir(), '.agents', 'cyberplace.json'))
})
...
test('readConfig returns stored config', () => {
	...
	fs.writeFileSync(
		path.join(root, '.agents', 'cyberplace.json'),
...
test('readConfig returns empty config on malformed JSON', () => {
	fs.mkdirSync(path.join(root, '.agents'), { recursive: true })
	fs.writeFileSync(path.join(root, '.agents', 'cyberplace.json'), 'not json')
...
test('writeConfig creates .agents dir and writes file', () => {
	...
	const filePath = path.join(root, '.agents', 'cyberplace.json')
...
test('writeConfig omits empty providers', () => {
	...
	const parsed = JSON.parse(fs.readFileSync(path.join(root, '.agents', 'cyberplace.json'), 'utf8'))
