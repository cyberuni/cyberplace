...
beforeEach(() => {
	root = fs.mkdtempSync(path.join(os.tmpdir(), 'cyberplace-find-'))
})
...
function makeMarketplaceResponse(count: number) {
	return {
		...
		skills: Array.from({ length: count }, (_, i) => ({
			id: `cyberuni/cyberplace/skill-${i}`,
			skillId: `skill-${i}`,
			name: `skill-${i}`,
			installs: (count - i) * 10,
...
const marketplaceResponse = {
	...
	skills: [
		...
		{
			id: 'cyberuni/cyberplace/add-changeset',
		...
		{
			id: 'cyberuni/cyberplace/create-skill',
...
test('findSkills includes install command', async () => {
	...
	const results = await findSkills('commit', { root })
	expect(results[0]!.installCommand).toContain('npx cyberplace add')
