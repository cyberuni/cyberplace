...
test('parseSpec: org/repo → repo spec', () => {
	const spec = parseSpec('cyberuni/cyberplace')
	expect(spec).toMatchObject({ type: 'repo', owner: 'cyberuni', repo: 'cyberplace' })
})

test('parseSpec: org/repo:skill → repo spec with skill', () => {
	const spec = parseSpec('cyberuni/cyberplace:commit')
	expect(spec).toMatchObject({ type: 'repo', owner: 'cyberuni', repo: 'cyberplace', skill: 'commit' })
})
...
test('parseSpec: GitHub bare HTTPS URL → git-url spec with no branch', () => {
	const spec = parseSpec('https://github.com/cyberuni/cyberplace')
	expect(spec).toMatchObject({
		type: 'git-url',
		owner: 'cyberuni',
		repo: 'cyberplace',
		cloneUrl: 'https://github.com/cyberuni/cyberplace',
		host: 'https://github.com',
		providerHint: null,
	})
...
test('parseSpec: HTTPS URL with .git suffix stripped', () => {
	const spec = parseSpec('https://github.com/cyberuni/cyberplace.git')
	expect(spec).toMatchObject({
		type: 'git-url',
		cloneUrl: 'https://github.com/cyberuni/cyberplace',
	})
...
test('parseSpec: bare HTTPS URL with #branch fragment', () => {
	const spec = parseSpec('https://github.com/cyberuni/cyberplace#develop')
	expect(spec).toMatchObject({
		type: 'git-url',
		branch: 'develop',
		cloneUrl: 'https://github.com/cyberuni/cyberplace',
...
test('parseSpec: SSH URL → git-url spec with no providerHint', () => {
	const spec = parseSpec('git@github.com:cyberuni/cyberplace.git')
	expect(spec).toMatchObject({
		type: 'git-url',
		owner: 'cyberuni',
		repo: 'cyberplace',
