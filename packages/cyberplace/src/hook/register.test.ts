...
test('matches hooks by --name and input flags', () => {
	const expected = "npx cyberplace@1.0.0 hook run --name local-augmentations --glob '.agents/skills/**/SKILL.local.md'"
	const existing = "npx cyberplace@1.0.0 hook run --name local-augmentations --glob '.agents/skills/**/SKILL.local.md'"
	expect(commandMatchesHook(existing, 'local-augmentations', expected)).toBe(true)
	expect(
		commandMatchesHook(
			"npx cyberplace@1.0.0 hook run --name other --glob '.agents/skills/**/SKILL.local.md'",
			'local-augmentations',
			expected,
		),
	).toBe(false)
