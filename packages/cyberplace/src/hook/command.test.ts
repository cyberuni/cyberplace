...
const npxExpected =
	"npx cyberplace@2.0.0 hook run --name commit-discipline --extract AGENTS.md --heading 'Commit Discipline'"
const pnpmExpected =
	"pnpm exec cyberplace hook run --name commit-discipline --extract AGENTS.md --heading 'Commit Discipline'"

let root: string

beforeEach(() => {
	root = fs.mkdtempSync(path.join(os.tmpdir(), 'cyberplace-hook-'))
})
...
test('hookCommand uses npx when no lock file present', () => {
	...
	expect(cmd).toMatch(/^npx cyberplace@/)
...
test('commandMatchesHook still compares semver for valid commands', () => {
	const existing =
		"npx cyberplace@1.0.0 hook run --name commit-discipline --extract AGENTS.md --heading 'Commit Discipline'"
...
test('commandMatchesHook returns false when pm exec prefix differs', () => {
	const existing =
		"yarn exec cyberplace hook run --name commit-discipline --extract AGENTS.md --heading 'Commit Discipline'"
	expect(commandMatchesHook(existing, 'commit-discipline', pnpmExpected)).toBe(false)
})
