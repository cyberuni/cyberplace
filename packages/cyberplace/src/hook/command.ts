...
export function hookCommand(subcommand: string, root = process.cwd()): string {
	...
	if (prefix) {
		return `${prefix} cyberplace ${subcommand}`
	}
	const version = getPackageVersion()
	return `npx cyberplace@${version} ${subcommand}`
}
...
function extractNpxVersion(command: unknown): string | null {
	if (!isCommandString(command)) return null
	const match = command.match(/npx(?:\s+(?:--yes|-y))?\s+cyberplace@(\S+)/)
	return match?.[1] ?? null
}
...
function extractPmExecPrefix(command: string): string | null {
	const match = command.match(/^(pnpm exec|yarn exec|bunx|npm exec)\s+cyberplace/)
	return match?.[1] ?? null
}
...
export function commandMatchesHook(existing: unknown, hookId: string, expectedCommand: string): boolean {
	...
	const existingUsesLocalBin = /node_modules[/\\]\.bin[/\\]cyberplace/.test(existing)
	const expectedUsesLocalBin = /node_modules[/\\]\.bin[/\\]cyberplace/.test(expectedCommand)
