...
export function getLayerFilePath(cwd: string, sourceClass: Exclude<SourceClass, 'default'>): string {
	switch (sourceClass) {
		case 'local-private':
...
export function parseRepositoryFromPackage(cwd: string): string | null {
...
export function loadSourceConfigFile(filePath: string): SourceConfigFile {
...
export function getResolvedSources(cwd: string): ResolvedSource[] {
...
async function loadAwesomeListFromSource(source: ResolvedSource, cwd: string): Promise<AwesomeListFile> {
	...
	const response = await fetch(`https://api.github.com/repos/${source.repo}/contents/${source.path}`, {
		headers: {
			...
			'User-Agent': 'cyberplace-awesome-skills',
		},
	})
	if (!response.ok)
