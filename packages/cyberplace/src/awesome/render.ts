...
export function renderAwesomeList(cwd: string): { readmePath: string; changed: boolean } {
	...
	const updated = updateMarkedSection(before, 'AWESOME-SKILLS', markdown)
	const changed = before !== updated
	if (changed) fs.writeFileSync(readmePath, updated)
	return { readmePath, changed }
}
