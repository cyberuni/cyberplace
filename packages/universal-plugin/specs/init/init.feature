Feature: init plugin project
  ...
  Scenario: --vendor adds vendorExtensions stub
    When I run "universal-plugin init --vendor claude-code --vendor cursor --yes --root <root>"
  ...
  Scenario: --scaffold creates standard directories
    When I run "universal-plugin init --scaffold --yes --root <root>"
    Then the exit code is 0
  ...
  Scenario: --force overwrites existing manifest
    Given ".plugin/plugin.json" already exists
    When I run "universal-plugin init --force --yes --root <root>"
    Then the exit code is 0
    And ".plugin/plugin.json" is overwritten

  Scenario: --format json returns list of created files
    When I run "universal-plugin init --yes --format json --root <root>"
    Then the exit code is 0
  ...
  Scenario: --format json suppresses interactive prompts
    When I run "universal-plugin init --format json --root <root>"
    Then the exit code is 0
