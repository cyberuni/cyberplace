...
| **Actor** | The narrow case of a **role**: one of the four motive-distinct base roles — Director, Builder, Architect, Strategist — and the human party in the **actor/delegate** pair. Holds accountability, which never delegates. *Collision: unlike a UML use-case "actor" (which may be a non-human system), our actor is **always human** — agents are delegates.* |
...
| **Motive** | The intrinsic **want** that defines an actor — *face-invariant*: both faces serve it. Named by its forward verb for brevity — **intend** (Director: the right thing exists), **generate** (Builder: the thing works), **structure** (Architect: the whole stays coherent and evolvable), **accumulate** (Strategist: knowledge compounds) — but the **backward face** pursues the same want by *judging against* it, not producing toward it. |
...
| **Face** | The *orientation* of an actor's expertise — **forward** (produce) or **backward** (evaluate). Every actor has both, serving one **motive**. Switching motive is switching actor, not face. |
| **Variant** | A specialization of an actor — same **motive**, differentiated by required *training* — that clears the **membership gates**. Anchors to *either face*: forward (**Explorer** of Builder, confirmed; **Scout** of Director, forming; **Conductor** of Architect, forming) or backward (**QA**, the Builder variant that evaluates the product, confirmed). **Strategist** has none. |
...
| **Angle** (domain) | The subject-matter expertise an actor works in — security, UX, performance, data, quality. An actor is **angle-agnostic**: it carries no angle of its own; the human supplies one, always acting *from an angle*. Orthogonal to *both* **actor** and the product/process/toolchain **sets** — and not to be confused with either. (Set-names like *process* or *toolchain* are not angles.) |
...
| **Lens** | SDD's term for an actor's **bar** in applied form — the Director (scope), Builder (coverage), or Architect (structure) governance resolved per artifact-type and applied at a gate. A producer self-aligns **forward**; a judge grades **backward**. The same criteria as the **bar**, named for the delegate that looks *through* it; not the **actor**, **face**, or **delegate**. |
...
| Single-/double-loop learning (the Strategist's two modes) | Argyris & Schön, *Organizational Learning* (1978). |
| The platform team as a self-service product (the Strategist tier) | Skelton & Pais, *Team Topologies*. |
