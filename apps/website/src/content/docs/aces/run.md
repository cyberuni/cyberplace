---
title: run
description: Score an ACES golden set against the current agent configuration and report results.
---
...
3. For each case in `golden-set/`, invokes the `aces-case-judge` subagent with the target's full text, the scenario, expected behaviors, must-not-do list, and rubric — skipping layers not enabled in `eval.md`.
