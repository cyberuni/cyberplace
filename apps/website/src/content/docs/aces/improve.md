---
title: improve
description: Diagnose failing ACES eval cases and propose targeted edits to the target agent configuration.
---

Part of the [ACES plugin](/aces/overview/) — see that page for install instructions.

**Trigger:** ACES evals are failing and the user wants to diagnose why and get specific improvement proposals

Diagnoses failing eval cases and proposes targeted edits to the target agent configuration.

## What it does

1. Loads `artifacts/specs/<feature-name>/eval.md`, the target agent configuration, and the most recent `results/` file (runs [`run`](/aces/run/) first if none exists).
...
6. After approval, applies the edits, then automatically runs [`compare`](/aces/compare/) (previous git revision vs. current working tree) to confirm improvement without regression.

## Role as ACES impl-producer

When dispatched by the SDD conductor (with [`define-agent`](/aces/define-agent/)) against a frozen `.feature`, this skill is the impl-producer for agent-config domains — it also authors or refreshes the eval suite (`eval.md` + `golden-set/`) the impl-judge runs, since the judge never authors evals itself.
