...
List skills from the repo, global install, and the `cyber-skills` package:

```bash
npx cyber-skills@<version> skill list
npx cyber-skills@<version> skill list --grep "init-*"
npx cyber-skills@<version> skill list --format agent
npx cyber-skills@<version> skill list --format json
```
...
```bash
npx cyber-skills@<version> skill validate-private
```
...
```bash
npx cyber-skills@<version> skill repair-private
```
...
```bash
npx cyber-skills@<version> skill source commit-work
```
