...
| **Variant** | A specialization of an actor's *forward face* that needs different preparation and clears the **membership gates**. Thrown off by two axes — *divergent/convergent* and *design-time/runtime*. **Explorer** (of Builder) is confirmed; **Scout** (of Framer) and **Conductor** (of Architect) are *forming*; **Curator** has none (its acts share one cognitive profile). |
