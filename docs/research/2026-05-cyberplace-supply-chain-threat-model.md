# cyberplace supply chain threat model
...
| --- | --- |
| Installing skills via [Skills CLI](https://github.com/vercel-labs/skills) | npm CVE audit of cyberplace dependencies |
| Running `cyberplace` via npx or local bin | Compromise of the user's machine or agent product |
...
flowchart TB
  subgraph external [External trust]
    GitHub["GitHub: cyberuni/cyberplace SKILL.md"]
    NpmRegistry["npm registry: cyberplace package"]
  end
...
**Scenario:** User runs `npx skills add cyberuni/cyberplace --skill init -g`. The Skills CLI fetches the repo default branch at install time. A compromised org, maintainer account, or malicious merge delivers trojaned `SKILL.md` before the user runs init.

**Impact:** Agent executes attacker instructions; may invoke shell, modify hooks, or steer toward `@latest` / unpinned tooling.

**Current mitigations:**

- Mechanical audit rules (E1–E2) in cyberplace-authored skills — does not protect consumers after hijack.
...
**Scenario:** During `init` / `init-commit-discipline`, the agent runs `npm view cyberplace version` and `npx cyberplace@<exact> …`. A bad publish at that moment gets pinned into hooks.
...
**Mitigations:** npm-aligned install (`pnpm add -D cyberplace` + `npx skills add ./node_modules/cyberplace …`) couples skill files and CLI to one release.
...
| **Team reproducible** | Project scope, commit `skills-lock.json`, `npx skills ci` | Pinned in hooks + optional `pnpm add -D cyberplace` | Shared repos, CI |
| **npm-aligned** | `npx skills add ./node_modules/cyberplace --skill init …` after devDependency | Same semver from `node_modules` | Strong coupling, offline CLI |
...
| --- | --- | --- |
| Pinned npx in init skills (never `@latest`) | T3 | cyberplace SKILL.md |
| Semver baked into SessionStart hooks | T3, T5 | `hook register` |
| Hook semver upgrade on re-register | T5 | cyberplace CLI |
| `skills-lock.json` + restore | T2, T6 | Consumer repo + Skills CLI |
| npm provenance on publish | T3 | cyberplace release |
| audit-skill before install | T1, T7 | Consumer workflow |
| npm-aligned devDependency path | T4 | Consumer workflow |

## Open questions

1. Should cyberplace ship a verified starter `skills-lock.json` fragment for init skills in consumer templates?
2. When Skills CLI stabilizes `skills ci --frozen-lockfile`, should init skills recommend it in AGENTS.md boilerplate?
3. Should `init` offer to add `cyberplace` as devDependency by default for Node repos?
