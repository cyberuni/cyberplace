...
cyber-skills ships version-pinned governances under `governances/` loadable via `governance show <name>`. Some standards belong in **domain repos** — for example `changeset-authoring` in [repobuddy/agent-changesets](https://github.com/repobuddy/agent-changesets) — not in the core cyber-skills package.
...
### Option 2: Repo manifest (`.cyber-skills/plugins.json`)
...
Domain repos publish npm packages with `governances/` and optional tiny CLI; cyber-skills adds federation later (`governance show <name> --package @scope/pkg` or clibuilder plugin).
...
### Option 4: clibuilder plugin on cyber-skills CLI immediately
...
**Defer external governance federation** in cyber-skills until domain skill repos ship as **npm packages** (or agentskills distribution RFC provides an equivalent manifest + digest contract).
...
2. cyber-skills core governances remain under `governances/` and load via `governance show`
...
Option 4 is valuable later if cyber-skills adopts clibuilder broadly; it is not required to unblock `changeset-authoring` living in agent-changesets.
...
- agent-changesets can land `governances/changeset-authoring.md` without waiting for cyber-skills CLI work
- Future federation aligns with ecosystem manifest/digest patterns

### Negative

- Split loading models temporarily (cyber-skills CLI vs co-located files in domain repos)
...
- Issue in `cyber-skills`: “External governance federation” referencing this ADR
...
**Pinned npx for cyber-skills itself** remains a repo/ADR concern — not a shipped governance — see research doc.
