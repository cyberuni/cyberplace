...
**Authors:** unional  
**Date:** 2026-06-13  
**Scope:** spec-driven quality system for subjects — skills, AGENTS.md sections, subagent definitions, commands

---

## 1. Problem Statement

Agent configuration subjects (skills, `AGENTS.md` sections, subagent definitions, commands) shape how AI agents behave. When these subjects are wrong or ambiguous, agents behave incorrectly — but there is no built-in way to detect this.
...
Existing tooling addresses only structure (`audit-skill` checks frontmatter and section presence) — not runtime behavior. There is no way to measure whether an subject actually produces correct agent behavior.

ACES applies spec-driven development (SDD) to subjects. In SDD, a spec (test cases + rubric) is written alongside the implementation and serves as the authoritative description of behavior. ACES does the same for subjects: the golden set is the spec, the subject is the implementation, and `run` is the verification step. The current scope focuses on the **backfill path** — adding specs to existing subjects — but the full SDD lifecycle (spec first, then implement) is the intended end state.

---

## 2. Goals

- **G1** — Define a test case format for subjects that captures scenario, expected behaviors, prohibited behaviors, and a scoring rubric.
- **G2** — Specify a golden set convention: a curated directory of test cases per subject that serves as ground truth.
...
- **G4** — Specify a regression gate: detect when an subject change drops scores below a threshold before committing.
- **G5** — Specify a report format for project-wide eval health across all subjects.
- **G6** — Specify an improvement workflow: map failing test cases to patterns, propose targeted edits to the subject.

## 3. Non-Goals

- **NG1** — Runtime monitoring of live agent sessions. ACES is an offline eval tool; online monitoring is a separate concern.
- **NG2** — Automated subject repair. ACES proposes edits; the user approves them. No auto-apply without confirmation.
- **NG3** — Evaluation of agent tool use or MCP server calls. ACES evaluates instruction-following behavior, not tool integration.
- **NG4** — Cross-subject conflict detection. Detecting that two skills give contradictory instructions is a future concern.
...
The collective term for all instruction subjects an agent runtime loads:
...
|---|---|---|---|
| **Structural** | Does the subject have required fields and format? | `audit-skill` (static) | — |
| **Trigger** | Does the agent correctly decide when to invoke this subject? | trigger rate (run-based) | `eval_queries.json` |
...
A single scenario that exercises one aspect of one subject at one layer. A test case specifies:
...
Assertions and rubric are complementary. Assertions check mechanical properties objectively ("output includes a bar chart file"); the rubric scores holistic quality. When assertions are present, a failed assertion caps the rubric score at 3 — the subject cannot score 4 or 5 if a must-pass check failed.

### 4.4 Golden set

The complete test suite for one subject:
...
A single execution of the eval suite against the current subject. Two run types:
...
A score cutoff (1–5) that classifies a test case as passing or failing. Default: 4. Configurable per subject in `eval.md` and overridable per test case in its frontmatter.

### 4.7 Regression gate

A check run by `aces-compare` that blocks an subject change if any test case's score drops vs. the previous version. A regression is a pass→fail flip or a score drop of ≥2 on any case.
...
The backfill path starts with an existing subject that has no spec yet.

```
create-spec
  └─ aces-spec-designer (per subject)
...
1. **`create-spec`** — scan for unevaluated subjects (or resolve a named one); invoke `aces-spec-designer` per subject; report file counts and structural issues.
...
4. **`improve`** (if failing cases) — invoke `aces-analyzer` to identify failure patterns; present proposed subject diffs; apply after user approval.
...
The forward path writes the spec before the subject exists — full SDD: spec first, then implement.

1. **`create-spec`** for a not-yet-implemented subject — generates behavioral spec from a description or requirements document.
2. **Implement** the subject (write `SKILL.md`, `AGENTS.md` section, etc.) to satisfy the golden set.
...
- **`run`** — re-run after any subject edit to catch regressions.
- **`compare`** — verify an subject change didn't regress before committing.
- **`report`** — project-wide health dashboard across all subject specs.
...
For subjects that belong to a plugin, nest under the plugin name:
...
**Trigger:** User asks to create evals / a spec for one or more subjects.

**Steps:**
1. Scan the project for subjects (skills, `AGENTS.md` sections, subagent definitions, commands) that have no `artifacts/aces/` entry yet
2. If a specific subject is named in the request, resolve it directly — skip scanning
3. If multiple unevaluated subjects are found, list them and ask the user to select one, several, or all
4. For each selected subject, invoke `aces-spec-designer` to perform analysis and generate the eval suite
5. Report: subjects processed, file counts, structural issues found, next step

**Output:** One `artifacts/aces/<subject-path>/` per selected subject, each populated and ready for `run`.

---

### 7.2 `run`

**Trigger:** User asks to run evals or validate after editing an subject.

**Steps:**
1. Locate `eval.md` (from user context or by scan)
2. Read the current subject in full
...
**Steps:**
1. Identify two versions: default is working tree vs. last git revision; `--baseline` flag compares with-subject vs. without-subject
...
**Trigger:** Eval results have failing cases; user wants to fix the subject.

**Steps:**
1. Load latest behavior result from `results/`, read failing test cases and `feedback.json`
2. Invoke `aces-analyzer` with failing cases, feedback notes, and current subject to extract improvement patterns
3. Group by pattern (see §9) and prioritize by impact
4. Present proposed diffs to the subject — do not auto-apply
5. After approval, apply edits and run `compare` to verify

**Output:** Proposed subject edits + post-edit comparison.
...
Invoked by `create-spec` for each selected subject. Analyzes the subject and produces the full eval suite.

**Input:**
```
ARTIFACT: <full subject text>
...
Invoked by `aces-run` for each behavior/quality test case. Simulates agent behavior given a scenario and subject, producing a structured output for grading.
...
**Output:** Simulated execution log — what the agent did step-by-step, what output it produced, whether it consulted the subject and which parts. Saved alongside each case result so `aces-grader` can examine the "transcript".
...
**Grading principles:**
- Score what the subject would cause an agent to do — not what the grader considers ideal
- The rubric is the authority; do not override it
- Ambiguous subject language that produces inconsistent behavior should lower the score
...
After all cases are judged, `aces-compare` unblids the results to attribute A/B to the actual subject versions.

### 8.5 `aces-analyzer`

Invoked by `aces-improve`. Given failing cases, `feedback.json`, and the current subject, produces prioritized improvement suggestions by examining which instruction gaps caused failures.
...
| Conflicting instruction | Agent follows one rule but violates another | Add explicit precedence rule; or split subject |
| Scope creep | Agent does more than the subject specifies | Add explicit scope boundary |
...
Agent behavior is not structurally checkable. The output of following an subject is natural language behavior, not a typed value. An LLM judge is the only practical scorer at the behavior and quality layers.
...
Test cases are committed to the repo alongside the subject. They are not regenerated each run.

**Tradeoff:** The golden set can go stale if the subject changes without updating the cases. This is a feature, not a bug — stale cases catch regressions. The author is expected to update the golden set when intentionally changing behavior.
...
Compare is a diff operation, not a recorded eval run. Writing a result for "before" would pollute the history with a result that doesn't reflect the current subject state.
