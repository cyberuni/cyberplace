...
export function runChecks(filePath: string): CheckResult {
  ...
  if (parent !== 'skills') {
    crit('S1', 'SKILL.md in own directory',
      `path: ${filePath}`,
  ...
  if (!fmName) {
    crit('S2', 'Required frontmatter: name',
      'name: field missing or empty',
      `Add 'name: ${dirName}' to the YAML frontmatter block`)
  }
  if (!fmDesc) {
    crit('S2', 'Required frontmatter: description',
  ...
  for (const pat of E1_PATTERNS) {
    ...
    if (hit) {
      crit('E1', 'Dangerous shell command (in code block)',
        hit.trim(),
  ...
  for (const pat of E2_PATTERNS) {
    ...
    if (hit) {
      crit('E2', 'Prompt injection pattern',
        hit.trim(),
  ...
  const e6Hit = stripped.split('\n').find(
    l => /git push.*(--force|-f )/.test(l) && /(main|master)/.test(l)
  )
  if (e6Hit) {
