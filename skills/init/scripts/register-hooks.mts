...
export function registerHooks(options: RegisterOptions = {}): HookResult[] {
  ...
  if (existsSync(claudeDir)) {
    const settings: ClaudeSettings = existsSync(claudeSettingsPath)
    ...
    if (commandExistsInGroups(settings.hooks.SessionStart, INJECT_AUGMENTATIONS)) {
      results.push({ agent: 'Claude Code', hook: 'SessionStart › inject-local-augmentations', status: 'already present' })
    } else {
  ...
  if (existsSync(cursorDir)) {
    const settings: CursorSettings = existsSync(cursorHooksPath)
      ? readJson(cursorHooksPath)
      : { version: 1 }
  ...
  if (existsSync(codexDir)) {
    const settings: ClaudeSettings = existsSync(codexHooksPath)
      ? readJson(codexHooksPath)
      : {}
