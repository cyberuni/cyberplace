#!/usr/bin/env node
import { findAwesomeSkills } from "./awesome-lib.mjs";
//#region src/skills/find-awesome-skill/scripts/find-awesome-skill.ts
function parseArgs(argv) {
	...
	return {
		query: args.filter((arg, index) => arg !== "--json" && index !== limitIdx && index !== limitIdx + 1).join(" ").trim(),
...
for (const result of results) {
	...
	if (result.corroborationCount > 1) console.log(`  Also recommended by ${result.corroborationCount - 1} other source(s).`);
