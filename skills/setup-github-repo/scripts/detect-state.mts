...
const repoSettings = runJson<RepoSettings>(
...
const defaultBranchRuleset = rulesets.find(
  (r) =>
    ...
    JSON.stringify(r.conditions)?.includes('~DEFAULT_BRANCH'),
)

// --- Existing workflows ---

const workflowsDir = '.github/workflows'
const existingWorkflows = existsSync(workflowsDir)
...
const state = {
  ...
  current: {
    ...
    dependabotSecurityUpdates:
      repoSettings.security_and_analysis?.dependabot_security_updates?.status ?? 'disabled',
    secretScanning:
      repoSettings.security_and_analysis?.secret_scanning?.status ?? 'disabled',
