#!/usr/bin/env node
import { flattenAwesomeEntries, validateAwesomeList } from "../../find-awesome-skill/scripts/awesome-lib.mjs";
import * as fs from "node:fs";
import * as path from "node:path";
import { pathToFileURL } from "node:url";
//#region src/skills/update-awesome-list/scripts/render-awesome-list.ts
...
function renderAwesomeListMarkdown(entries) {
	const sections = ["## Awesome Skills", ""];
	for (const trust of ["authored", "recommended"]) {
		const grouped = entries.filter((entry) => entry.trust === trust).sort((a, b) => a.repo.localeCompare(b.repo) || (a.type === "skill" ? a.skill : "").localeCompare(b.type === "skill" ? b.skill : ""));
		...
		for (const entry of grouped) {
			...
			if (entry.type === "repo" && entry.highlights && entry.highlights.length > 0) {
				...
				for (const highlight of entry.highlights) sections.push(`  - \`${highlight.type}:${highlight.key}\` — ${highlight.summary}`);
			}
			sections.push("");
		}
	}
...
function updateMarkedSection(content, markerName, replacement) {
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
	...
	const markdown = renderAwesomeListMarkdown(flattenAwesomeEntries(validateAwesomeList(JSON.parse(fs.readFileSync(awesomePath, "utf8")), awesomePath)));
...
//#endregion
export { renderAwesomeListMarkdown };
